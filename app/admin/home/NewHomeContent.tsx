...
function resolveRoleLabel(user: unknown) {
  const record = (user ?? {}) as Record<string, unknown>;
  const role = normalizeText(String(record.permissionRole ?? record.role ?? record.companyRole ?? "usuario"));
  if (role.includes("leader") || role.includes("lider")) return "Líder TC";
  if (role.includes("support") || role.includes("suporte") || role.includes("technical")) return "Suporte Técnico";
  if (role.includes("empresa") || role.includes("company")) return "Empresa";
  if (role.includes("testing") || role.includes("qa")) return "QA";
  return "Usuário";
}
...
function resolveVoice(voices: SpeechSynthesisVoice[], selectedName: string) {
  return voices.find((voice) => voice.name === selectedName) ?? voices.find((voice) => voice.lang === "pt-BR" && /female|maria|luciana|francisca|google/i.test(voice.name)) ?? voices.find((voice) => voice.lang === "pt-BR") ?? voices.find((voice) => voice.lang.startsWith("pt")) ?? null;
}
...
function buildAssistantReply(prompt: string, roleLabel: string) {
  return `Entendi. Vou seguir pelo contexto: ${prompt}. Para ${roleLabel}, eu posso abrir a área certa, consultar dados disponíveis, filtrar pelo seu perfil e manter o histórico desta conversa para continuar daqui.`;
}

function buildInitialMessage(greeting: string, firstName: string, roleLabel: string) {
  return `${greeting}, ${firstName}. Vamos iniciar por onde hoje? Eu posso trabalhar com o que está disponível para seu perfil de ${roleLabel}. Escolha uma opção, digite uma mensagem ou fale comigo por áudio.`;
}

function createSession(initialText: string): BrainSession {
  const now = new Date().toISOString();
  return {
    id: `brain-session-${Date.now()}`,
    title: "Nova conversa",
    createdAt: now,
    updatedAt: now,
    messages: [{ id: `assistant-${Date.now()}`, role: "assistant", text: initialText }],
  };
}

function titleFromMessages(messages: BrainMessage[]) {
  const firstUserMessage = messages.find((message) => message.role === "user")?.text;
  return firstUserMessage ? firstUserMessage.slice(0, 48) : "Conversa do Brain";
}
...
export default function NewHomeContent() {
  ...
  useEffect(() => {
    const today = todayBR();
    const storedDay = window.localStorage.getItem(DAILY_KEY);
    const stored = window.localStorage.getItem(STORAGE_KEY);
    const parsed = stored ? (JSON.parse(stored) as BrainSession[]) : [];
    const shouldStartFresh = storedDay !== today || parsed.length === 0;
    const nextSessions = shouldStartFresh ? [createSession(initialAssistantText), ...parsed] : parsed;
  ...
  useEffect(() => {
    if (voiceEnabled) speakBrain(latestAssistantMessage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [latestAssistantMessage, voiceEnabled, selectedVoice, volume]);

  function updateCurrentSession(updater: (session: BrainSession) => BrainSession) {
    setSessions((current) => current.map((session) => (session.id === currentSessionId ? updater(session) : session)));
  }
  ...
  function sendPrompt(prompt: string) {
    ...
    if (!cleanPrompt || !currentSessionId) return;
    const now = Date.now();
    const assistantText = buildAssistantReply(cleanPrompt, roleLabel);
    updateCurrentSession((session) => ({
      ...session,
      title: titleFromMessages([...session.messages, { id: `user-${now}`, role: "user", text: cleanPrompt }]),
      updatedAt: new Date().toISOString(),
      messages: [...session.messages, { id: `user-${now}`, role: "user", text: cleanPrompt }, { id: `assistant-${now}`, role: "assistant", text: assistantText }],
    }));
  ...
  function startNewConversation() {
    const next = createSession(initialAssistantText);
  ...
  function handleVoiceInput(sendDirectly: boolean) {
    ...
    if (!SpeechRecognition) {
      ...
      updateCurrentSession((session) => ({ ...session, messages: [...session.messages, { id: `assistant-speech-${Date.now()}`, role: "assistant", text: "Seu navegador não liberou reconhecimento de voz agora. Digite a mensagem ou use o chat lateral." }] }));
      return;
    }
    ...
    recognition.onerror = () => {
      ...
      updateCurrentSession((session) => ({ ...session, messages: [...session.messages, { id: `assistant-error-${Date.now()}`, role: "assistant", text: "Não consegui captar o áudio. Tente novamente ou digite sua mensagem." }] }));
    };
    recognition.onend = () => {
      setListening(false);
      setDirectVoiceMode(false);
  ...
  return (
    <section className="admin-brain-home relative min-h-[calc(100vh-7rem)] w-full overflow-hidden bg-transparent px-4 pb-28 pt-5 sm:px-8 lg:px-10">
            ...
            {messages.map((message, index) => {
              ...
              return (
                <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`admin-brain-message max-w-[82%] rounded-3xl px-5 py-4 ${message.role === "user" ? "admin-brain-message-user text-right" : "admin-brain-message-assistant"}`}>
                    <p className="whitespace-pre-wrap text-base font-semibold leading-relaxed sm:text-xl">{displayText}{isLatestAssistant ? <motion.span className="ml-1 inline-block h-5 w-[2px] translate-y-1 bg-red-500" animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 0.9 }} /> : null}</p>
                  </div>
                </div>
              );
            })}
      ...
      {historyOpen ? (
        <div className="admin-brain-history fixed bottom-[9.4rem] left-[13.25rem] z-40 w-[min(360px,calc(100vw-2rem))] rounded-3xl border border-white/10 bg-slate-950/90 p-3 shadow-2xl backdrop-blur-xl max-md:left-4">
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Histórico</p>
          <div className="max-h-64 space-y-2 overflow-y-auto pr-1">
            {sessions.map((session) => (
              <button key={session.id} type="button" onClick={() => openSession(session.id)} className={`w-full rounded-2xl border px-3 py-2 text-left transition ${session.id === currentSessionId ? "border-red-400/40 bg-red-500/10" : "border-white/10 bg-white/[0.035] hover:border-white/25"}`}>
                <span className="block truncate text-xs font-semibold text-white">{session.title}</span>
                <span className="mt-0.5 block text-[10px] text-slate-400">{new Date(session.updatedAt).toLocaleString("pt-BR")}</span>
              </button>
            ))}
          </div>
        </div>
