...
const userTypes: Array<{ value: UserType; label: string; help: string; requireCompany?: boolean }> = [
  ...
  { value: "technical_support", label: "Administrador", help: "Lista administrador e mostra tudo que ele movimentou: chamados, comentários, status e ações em módulos." },
...
function StatCard({ icon: Icon, value, label }: Readonly<{ icon: IconType; value: string | number; label: string }>) {
...
function RoundUserAvatar({ src, name, size = "md" }: Readonly<{ src?: string | null; name: string; size?: "sm" | "md" }>) {
...
function RoundCompanyAvatar({ company }: Readonly<{ company?: CompanyRow | null }>) {
...
export default function VisaoGeralComUsuarios() {
  ...
  const userLabel = selectedUser ? nameFromEmail(selectedUser) : roleLabel(userType);
  const companySuffix = company ? ` · ${company.name}` : "";
  const title = mode === "user" ? `${userLabel}${companySuffix}` : company?.name ?? "Operação geral";
