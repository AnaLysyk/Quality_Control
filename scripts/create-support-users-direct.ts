...
async function upsertSupportUsersAndSendWelcomeEmails() {
  log(colors.bright + colors.blue, "\n🚀 Criando/atualizando usuários de suporte técnico\n");
  ...
  for (const seed of supportUsers) {
    ...
    if (existing) {
      log(colors.yellow, `🔄 Atualizado: ${fullName} (${email})`);
    } else {
      log(colors.green, `✅ Criado: ${fullName} (${email})`);
    }

    const sent = await emailService.sendWelcomeEmail(email, login, tempPassword, fullName);
    if (sent) {
      log(colors.green, `📧 E-mail enviado para ${email}`);
    } else {
      log(colors.red, `❌ Falha ao enviar e-mail para ${email}`);
    }
  }

  log(colors.bright + colors.cyan, "\n══════════════════════════════════════════════════════════════════════\n");
  log(colors.bright + colors.green, "✅ Processo finalizado com sucesso.\n");
}

upsertSupportUsersAndSendWelcomeEmails().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error);
  log(colors.red, `❌ Erro fatal: ${message}`);
  process.exit(1);
}).finally(async () => {
