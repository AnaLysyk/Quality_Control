  ...
  return (
    <div className="w-full bg-(--page-bg,#f3f6fb) px-4 pt-4 pb-8 sm:px-6 lg:px-10 xl:px-12 2xl:px-14">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_minmax(320px,0.9fr)]">
        <div className="rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6">
        ...
        <div className="grid gap-4">
          {Array.from({ length: 2 }).map((_, index) => (
            <div key={index} className="h-48 animate-pulse rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)]" />
          ))}
        </div>
      </div>
    </div>
...
function RunnerSectionSkeleton() {
  return (
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)]">
      <div className="rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6 shadow-sm">
      ...
      <div className="space-y-4">
        {Array.from({ length: 2 }).map((_, index) => (
          <div key={index} className="h-64 animate-pulse rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)]" />
        ))}
      </div>
    </section>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="h-72 animate-pulse rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)]" />
        ))}
      </div>
      <div className="grid gap-4 xl:grid-cols-2">
        {Array.from({ length: 2 }).map((_, index) => (
          <div key={index} className="h-80 animate-pulse rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)]" />
        ))}
      </div>
    </div>
  );
...
function DeferredRunnerPlaceholder({ onLoad }: { onLoad: () => void }) {
  return (
    <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)]">
      <article className="rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6 shadow-sm">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={onLoad}
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[var(--tc-primary,#011848)] px-5 py-2 text-sm font-semibold text-[#011848] dark:text-white transition hover:opacity-90"
          >
      ...
      <aside className="rounded-[30px] border border-dashed border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6 shadow-sm">
        <div className="h-3 w-28 animate-pulse rounded-full bg-slate-200" />
      ...
      <div className="w-full bg-(--page-bg,#f3f6fb) px-4 pt-4 pb-8 text-(--page-text,#0b1a3c) sm:px-6 lg:px-10 xl:px-12 2xl:px-14">
        {!access.canOpen ? (
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
            <section className="rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6 shadow-sm">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link
                  href="/docs"
                  className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[var(--tc-border,#d7deea)] bg-white px-5 py-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)] transition hover:border-[var(--tc-accent,#ef0001)] hover:text-[var(--tc-accent,#ef0001)]"
                >
                  Ver base documental
                </Link>
              </div>
            ...
            <aside className="rounded-[30px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {[
                  ...
                  <div
                    key={item}
                    className="rounded-2xl border border-[var(--tc-border,#e5e7eb)] bg-[var(--tc-surface-2,#f8fafc)] px-4 py-3 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]"
                  >
                    {item}
                  </div>
                ))}
