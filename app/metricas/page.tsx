...
export default function MetricasPage() {
  ...
  useEffect(() => {
    ...
    if (activeClientSlug) {
      router.replace(`/empresas/${encodeURIComponent(activeClientSlug)}/metricas`);
      return;
    }

    if (clients.length === 1) {
      setActiveClientSlug(clients[0].slug);
      router.replace(`/empresas/${encodeURIComponent(clients[0].slug)}/metricas`);
      return;
    }
              ...
              {clients.map((client) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    router.push(`/empresas/${encodeURIComponent(client.slug)}/metricas`);
                  }}
                >
