...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (provider !== "qase" && provider !== "jira") return NextResponse.json({ error: "provider inválido" }, { status: 400 });
  ...
  const company = await prisma.company.findUnique({
    where: { slug },
    select: { id: true, name: true, qase_token: true, jira_base_url: true, jira_email: true, jira_api_token: true },
  });
  ...
  const projects = await prisma.project.findMany({
    ...
    orderBy: { name: "asc" },
    select: { id: true, name: true, slug: true, qaseProjectCode: true, jiraProjectKey: true, manualCreationDisabled: true },
  });

  try {
    const externalProjects = provider === "qase"
      ? company.qase_token ? await listQaseProjects(company.qase_token) : []
    ...
    return NextResponse.json({
      provider,
      company: { id: company.id, name: company.name, slug, configured: provider === "qase" ? Boolean(company.qase_token) : Boolean(company.jira_base_url && company.jira_email && company.jira_api_token) },
      projects,
      externalProjects: externalProjects.map((external) => ({
        ...external,
        linkedProject: projects.find((project) => provider === "qase" ? project.qaseProjectCode === external.key : project.jiraProjectKey === external.key) ?? null,
      })),
    });
  } catch (error) {
    const status = error instanceof QaseError ? error.status : 502;
    return NextResponse.json({ error: error instanceof Error ? error.message : "Falha ao consultar integração" }, { status });
  }
}
...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (input.provider === "qase" && !input.confirmIntegratedRepository) {
    return NextResponse.json({ error: "Confirme que o projeto passará a usar o repositório integrado do Qase." }, { status: 400 });
  }
  ...
  const duplicate = await prisma.project.findFirst({
    where: { companyId: company.id, ...(input.provider === "qase" ? { qaseProjectCode: input.externalKey.toUpperCase() } : { jiraProjectKey: input.externalKey.toUpperCase() }) },
    select: { id: true, name: true },
  });
  if (duplicate && duplicate.id !== input.projectId) return NextResponse.json({ error: `Integração já vinculada ao projeto ${duplicate.name}.` }, { status: 409 });
  ...
  if (!projectId || input.createProject) {
    ...
    let suffix = 2;
    while (await prisma.project.findUnique({ where: { companyId_slug: { companyId: company.id, slug: projectSlug } }, select: { id: true } })) {
      projectSlug = `${baseSlug}-${suffix++}`;
    }
    const created = await prisma.project.create({
      data: {
        companyId: company.id,
        name: input.externalName || input.externalKey.toUpperCase(),
        slug: projectSlug,
    ...
    const project = await prisma.project.findFirst({ where: { id: projectId, companyId: company.id }, select: { id: true } });
    if (!project) return NextResponse.json({ error: "Projeto interno não encontrado nesta empresa" }, { status: 404 });
  ...
  return NextResponse.json({ ok: true, projectId, provider: input.provider, externalKey: input.externalKey.toUpperCase() });
}
