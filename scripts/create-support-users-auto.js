...
async function createSupportUsers(adminToken) {
  ...
  if (failureCount > 0) {
    ...
    log(colors.white, '      node scripts/create-support-users-auto.js <TOKEN_ADMIN>\n');
  }
}
...
if (!adminToken) {
  ...
  log(colors.white, '  ADMIN_TOKEN=sua_senha node scripts/create-support-users-auto.js\n');
  process.exit(1);
}
