...
export default function AutomationCasesBoard({ access, activeCompanySlug, companies }: Props) {
  ...
  return (
    ...
    >
      ...
      <header className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[linear-gradient(135deg,#011848,#162b68)] p-5 text-white shadow-sm">
      ...
      <article className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
        ...
        <div className="mt-4 grid auto-rows-fr gap-3 grid-cols-[repeat(auto-fit,minmax(180px,1fr))]">
          {metrics.map((item) => {
            ...
            return (
              <div key={item.label} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-4 py-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--tc-text-muted,#6b7280)]">{item.label}</p>
                  <Icon className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" />
                </div>
                <p className="mt-2 text-lg font-black tracking-[-0.03em] text-[var(--tc-text,#0b1a3c)]">{item.value}</p>
                <p className="mt-1 text-xs text-[var(--tc-text-secondary,#4b5563)]">{item.hint}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid items-start gap-4 xl:grid-cols-12">
        <aside className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-4 xl:col-span-4 xl:sticky xl:top-6 2xl:col-span-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--tc-text-muted,#6b7280)]">Casos</p>
              <p className="mt-2 text-sm text-[var(--tc-text-secondary,#4b5563)]">{filteredCases.length} resultado(s)</p>
            </div>
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--tc-text-muted,#6b7280)]">{testCase.application}</p>
                        <h3 className="mt-1 wrap-break-word text-base font-black tracking-[-0.03em] text-[var(--tc-text,#0b1a3c)]">{testCase.title}</h3>
                      </div>
                    ...
                    <p className="mt-2 text-sm text-[var(--tc-text-secondary,#4b5563)]">{testCase.summary}</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <span className="inline-flex rounded-full border border-[var(--tc-border,#d7deea)] bg-white px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--tc-text,#0b1a3c)]">
        ...
        <article className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-5 xl:col-span-8 2xl:col-span-9">
          {selectedCase ? (
            <>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--tc-accent,#ef0001)]">Caso selecionado</p>
                  <h2 className="mt-2 text-2xl font-black tracking-[-0.03em] text-[var(--tc-text,#0b1a3c)]">{selectedCase.title}</h2>
                  <p className="mt-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">{selectedCase.objective}</p>
                  <p className="mt-1 text-sm text-[var(--tc-text-secondary,#4b5563)]">{selectedCase.summary}</p>
                </div>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="mt-5 grid items-start gap-4 2xl:grid-cols-[minmax(0,1fr)_340px]">
                <div className="space-y-4">
                  <section>
                    ...
                    <p className="mt-2 text-sm leading-7 text-[var(--tc-text,#0b1a3c)]">{selectedCase.expectedResult}</p>
                  </section>
                  ...
                  <section>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {selectedCase.preconditions.map((item) => (
                        <span
                          key={item}
                          className="inline-flex items-center rounded-full border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-3 py-1 text-xs font-semibold text-[var(--tc-text,#0b1a3c)]"
                        >
                          {item}
                        </span>
                      ))}
                ...
                <aside className="space-y-4">
                  <section className="rounded-3xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
                    ...
                    <p className="mt-2 text-lg font-black tracking-[-0.03em] text-[var(--tc-text,#0b1a3c)]">{selectedFlow?.title ?? "Sem fluxo"}</p>
                    <p className="mt-1 text-sm text-[var(--tc-text-secondary,#4b5563)]">{selectedFlow?.description ?? "Caso ainda sem fluxo publicado."}</p>
                    {selectedFlow ? (
                      <div className="mt-3 grid gap-2">
                        <div className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-3 py-2 text-sm text-[var(--tc-text,#0b1a3c)]">
                          <strong>{selectedFlow.steps.length}</strong> etapas no fluxo
                        </div>
                        <div className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-3 py-2 text-sm text-[var(--tc-text,#0b1a3c)]">{selectedFlow.stack}</div>
                      </div>
                    ) : null}
                  </section>
                ...
                <div className="mt-5">
                  ...
                  <div className="mt-2 grid gap-2">
                    {selectedFlow.steps.map((step, index) => (
                      <div key={`${selectedFlow.id}-${index}`} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-4 py-3 text-sm text-[var(--tc-text,#0b1a3c)]">
                        <span className="font-semibold text-[var(--tc-accent,#ef0001)]">#{index + 1}</span> {step.title}
                      </div>
                    ))}
                  </div>
                </div>
