...
function getFocusGraph(nodes: BrainNode[], edges: BrainEdge[], selectedNodeId: string | null) {
  ...
  const focusedNodes = nodes.filter((node) => ids.has(node.id));
...
function layoutNodes(nodes: BrainNode[], edges: BrainEdge[], selectedNodeId: string | null, saved: Record<string, { x: number; y: number }>, canvasSize: { width: number; height: number }, themeMode: "light" | "dark") {
  ...
  const bucketFor = (node: BrainNode) => {
    const text = [node.type, node.module, node.label, node.refType, node.description].filter(Boolean).join(" ").toLowerCase();

    if (node.id === preferredCenter?.id) return "center";
    if (text.includes("company") || text.includes("empresa")) return "company";
    if (text.includes("project") || text.includes("projeto") || text.includes("application")) return "project";
    if (text.includes("user") || text.includes("usuario") || text.includes("ana") || text.includes("perfil")) return "people";
    if (text.includes("wiki") || text.includes("doc") || text.includes("document")) return "knowledge";
    if (text.includes("test") || text.includes("run") || text.includes("case") || text.includes("plan")) return "qa";
    return "other";
  };

  const buckets: Record<string, BrainNode[]> = {
    center: [],
    company: [],
    project: [],
    people: [],
  ...
  for (const node of nodes) {
    buckets[bucketFor(node)].push(node);
  }

  Object.values(buckets).forEach((items) => {
    items.sort((left, right) => degree(right.id) - degree(left.id) || left.label.localeCompare(right.label, "pt-BR"));
  });

  const positions = new Map<string, { x: number; y: number }>();
  const spread = (count: number, index: number, gap: number) => (index - (count - 1) / 2) * gap;
  const verticalGap = 136;
  const horizontalGap = Math.min(360, Math.max(250, canvasSize.width / 4.7));

  if (preferredCenter) {
    positions.set(preferredCenter.id, center);
  }

  const columns: Record<string, { x: number; y: number; gap: number }> = {
    people: { x: center.x - horizontalGap * 1.15, y: center.y, gap: verticalGap },
    company: { x: center.x - horizontalGap * 0.45, y: center.y, gap: verticalGap },
    project: { x: center.x + horizontalGap * 0.45, y: center.y, gap: verticalGap },
    qa: { x: center.x + horizontalGap * 1.18, y: center.y, gap: verticalGap },
    knowledge: { x: center.x, y: center.y + verticalGap * 1.65, gap: verticalGap * 0.92 },
    other: { x: center.x, y: center.y - verticalGap * 1.65, gap: verticalGap * 0.92 },
  };

  for (const [bucket, items] of Object.entries(buckets)) {
    if (bucket === "center") continue;

    const column = columns[bucket] ?? columns.other;
    items.forEach((node, index) => {
      positions.set(node.id, {
        x: column.x,
        y: column.y + spread(items.length, index, column.gap),
      });
    });
  }
  ...
  return nodes.map<Node>((node) => {
    ...
    return {
      id: node.id,
      type: "brainNeuron",
      position: saved[node.id] ?? positions.get(node.id) ?? center,
      data: nodeData,
      draggable: true,
    };
  });
...
export function BrainNeuralCanvas({ nodes, edges, selectedNodeId, onSelectNode, onOpenRelatedModule, localGraphOnly, onToggleLocalGraph, loading = false, debugMode = false, onBackNode, canBackNode = false }: BrainNeuralCanvasProps) {
