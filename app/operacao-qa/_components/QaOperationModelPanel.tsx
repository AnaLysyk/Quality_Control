...
function SectionCard({ children }: { children: React.ReactNode }) {
  return <article className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4 shadow-sm">{children}</article>;
}
...
export function QaOperationModelPanel() {
  ...
  return (
    <main className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">
              <FiGitBranch className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Modelo operacional de QA
            </span>
            <h1 className="mt-3 text-2xl font-black text-(--tc-text,#0b1a3c)">Projeto → Repositório → Plano → Run → Métricas → Brian</h1>
          ...
          <div className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-4 py-3 text-sm font-bold text-(--tc-text,#0b1a3c)">
          ...
          {tabs.map((item) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-bold ${active ? "border-(--tc-accent,#ef0001) bg-(--tc-accent,#ef0001) text-white" : "border-(--tc-border,#d7deea) bg-white text-(--tc-text,#0b1a3c)"}`}
              >
          ...
          {data?.hierarchy.map((item, index) => (
            <SectionCard key={item.id}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{String(index + 1).padStart(2, "0")} · {item.kind}</p>
                  <h2 className="mt-1 text-lg font-black text-(--tc-text,#0b1a3c)">{item.title}</h2>
                  <p className="mt-1 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{item.description}</p>
                </div>
                <Badge>{item.required ? "Obrigatório" : "Opcional"}</Badge>
              </div>
              <p className="mt-3 rounded-xl border border-(--tc-border,#d7deea) bg-white p-3 text-sm font-semibold text-(--tc-text,#0b1a3c)">Vazio: {item.emptyState}</p>
              <div className="mt-3 flex flex-wrap gap-2">{item.routes.map((route) => <Badge key={route}>{route}</Badge>)}</div>
              <ul className="mt-3 space-y-1 text-xs leading-5 text-(--tc-text-secondary,#4b5563)">{item.rules.map((rule) => <li key={rule}>• {rule}</li>)}</ul>
            </SectionCard>
          ))}
        </section>
      ) : null}
          ...
          {data?.automationStatusFlow.map((item) => (
            <SectionCard key={item.status}>
              <div className="flex items-center justify-between gap-3">
                <h2 className="text-lg font-black text-(--tc-text,#0b1a3c)">{item.label}</h2>
                <Badge>{item.visibleInAutomation ? "Aparece na automação" : "Só manual"}</Badge>
              </div>
              <p className="mt-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{item.meaning}</p>
              <p className="mt-3 font-mono text-xs font-bold text-(--tc-accent,#ef0001)">{item.status}</p>
            </SectionCard>
          ))}
        </section>
      ) : null}
      ...
      {tab === "runs" ? (
        <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
          <h2 className="text-xl font-black text-(--tc-text,#0b1a3c)">Lifecycle obrigatório da execução</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {data?.runLifecycle.map((step, index) => (
              <div key={step} className="flex gap-3 rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-3">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-(--tc-accent,#ef0001) text-sm font-black text-white">{index + 1}</span>
                <p className="text-sm font-semibold leading-6 text-(--tc-text,#0b1a3c)">{step}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          {data?.metrics.map((metric) => (
            <SectionCard key={metric.id}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{metric.scope}</p>
                  <h2 className="mt-1 text-lg font-black text-(--tc-text,#0b1a3c)">{metric.label}</h2>
                </div>
                <FiActivity className="h-5 w-5 text-(--tc-accent,#ef0001)" />
              </div>
              <p className="mt-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{metric.description}</p>
              <p className="mt-3 rounded-xl border border-(--tc-border,#d7deea) bg-white p-3 font-mono text-xs leading-5 text-(--tc-text,#0b1a3c)">{metric.formula}</p>
        ...
        <section className="grid gap-3 lg:grid-cols-2">
          {data?.brianCommands.map((command) => (
            <SectionCard key={command.id}>
              <h2 className="text-lg font-black text-(--tc-text,#0b1a3c)">{command.label}</h2>
              <p className="mt-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{command.intent}</p>
              <p className="mt-3 rounded-xl border border-(--tc-border,#d7deea) bg-white p-3 text-sm font-bold text-(--tc-text,#0b1a3c)">“{command.example}”</p>
              <div className="mt-3 flex flex-wrap gap-2">{command.requiredContext.map((context) => <Badge key={context}>{context}</Badge>)}</div>
              <ul className="mt-3 space-y-1 text-xs leading-5 text-(--tc-text-secondary,#4b5563)">{command.guardrails.map((rule) => <li key={rule}>• {rule}</li>)}</ul>
            </SectionCard>
          ))}
        </section>
      ) : null}
          ...
          {data?.backlog.map((item) => (
            <SectionCard key={item.id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{item.area}</p>
                  <h2 className="mt-1 text-lg font-black text-(--tc-text,#0b1a3c)">{item.title}</h2>
                </div>
                <div className="flex gap-2"><Badge>{item.priority}</Badge><Badge>{item.status}</Badge></div>
              </div>
              <ul className="mt-3 space-y-1 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{item.acceptanceCriteria.map((criteria) => <li key={criteria}>• {criteria}</li>)}</ul>
            </SectionCard>
          ))}
        </section>
      ) : null}
