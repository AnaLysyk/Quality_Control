import { SYSTEM_ROLES, type SystemRole } from "@/lib/auth/roles";
import type { SystemRouteDefinition } from "./navigation.types";

const ALL_PROFILES: SystemRole[] = [
  SYSTEM_ROLES.LEADER_TC,
  SYSTEM_ROLES.TECHNICAL_SUPPORT,
  SYSTEM_ROLES.TESTING_COMPANY_USER,
  SYSTEM_ROLES.EMPRESA,
  SYSTEM_ROLES.COMPANY_USER,
];

const INTERNAL_PROFILES: SystemRole[] = [
  SYSTEM_ROLES.LEADER_TC,
  SYSTEM_ROLES.TECHNICAL_SUPPORT,
  SYSTEM_ROLES.TESTING_COMPANY_USER,
];

const ADMIN_PROFILES: SystemRole[] = [
  SYSTEM_ROLES.LEADER_TC,
  SYSTEM_ROLES.TECHNICAL_SUPPORT,
];

const COMPANY_PROFILES: SystemRole[] = [
  SYSTEM_ROLES.LEADER_TC,
  SYSTEM_ROLES.TECHNICAL_SUPPORT,
  SYSTEM_ROLES.TESTING_COMPANY_USER,
  SYSTEM_ROLES.EMPRESA,
  SYSTEM_ROLES.COMPANY_USER,
];

function formatSupplementalRouteLabel(pathname: string) {
  const segments = pathname.split("/").filter(Boolean);
  const rawSegment = segments[segments.length - 1] ?? "tela";

  if (/^\[.+\]$/.test(rawSegment)) {
    const previousSegment = segments[segments.length - 2] ?? "detalhe";
    return `${previousSegment.replace(/-/g, " ")} detalhe`;
  }

  return rawSegment.replace(/-/g, " ");
}

function getSupplementalModuleId(pathname: string): SystemRouteDefinition["moduleId"] {
  if (pathname.startsWith("/admin/requests") || pathname === "/requests") return "solicitacoes";
  if (pathname.startsWith("/admin/support") || pathname.includes("/chamados") || pathname === "/issues" || pathname === "/kanban-it") return "suporte";
  if (pathname.startsWith("/admin/runs") || pathname.startsWith("/admin/releases") || pathname.startsWith("/admin/defeitos")) return "testes-manuais";
  if (pathname.startsWith("/admin/docs")) return "documentos";
  if (pathname.startsWith("/admin/audit-logs") || pathname.startsWith("/admin/alerts")) return "configuracoes";
  if (pathname.startsWith("/admin/home") || pathname === "/admin" || pathname.startsWith("/admin/test-metric")) return "dashboards";
  if (pathname.startsWith("/admin")) return "configuracoes";
  if (pathname.startsWith("/agenda")) return "agenda";
  if (pathname.startsWith("/applications")) return "empresas";
  if (pathname.startsWith("/automacoes")) return "automacao";
  if (pathname.startsWith("/brain")) return "brain";
  if (pathname === "/brand-identity" || pathname === "/central-de-qualidade" || pathname === "/modelo-qualidade") return "documentos";
  if (pathname === "/clients" || pathname === "/clients-list" || pathname === "/empresas" || pathname.startsWith("/empresas")) return "empresas";
  if (pathname.startsWith("/dashboard") || pathname === "/metrics") return "dashboards";
  if (pathname === "/docs" || pathname === "/documentacao") return "documentos";
  if (pathname === "/issues" || pathname === "/notificacoes") return "suporte";
  if (pathname === "/me" || pathname.startsWith("/settings")) return "configuracoes";
  if (pathname.startsWith("/release") || pathname.startsWith("/runs")) return "testes-manuais";
  return "configuracoes";
}

function getSupplementalPermission(moduleId: SystemRouteDefinition["moduleId"]) {
  switch (moduleId) {
    case "empresas":
      return { moduleId: "applications", action: "view" } as const;
    case "usuarios":
      return { moduleId: "users", action: "view" } as const;
    case "permissoes":
      return { moduleId: "permissions", action: "view" } as const;
    case "testes-manuais":
      return { moduleId: "test_repository", action: "read" } as const;
    case "automacao":
      return { moduleId: "playwright", action: "read" } as const;
    case "brain":
      return { moduleId: "brain", action: "view" } as const;
    case "assistente":
      return { moduleId: "ai", action: "use" } as const;
    case "chat":
      return { moduleId: "chat", action: "view" } as const;
    case "suporte":
      return { moduleId: "support", action: "view" } as const;
    case "solicitacoes":
      return { moduleId: "access_requests", action: "view" } as const;
    case "documentos":
      return { moduleId: "documents", action: "view" } as const;
    case "dashboards":
      return { moduleId: "dashboard", action: "view" } as const;
    case "operacao":
      return { moduleId: "operations", action: "view" } as const;
    case "agenda":
      return { moduleId: "release_calendar", action: "view" } as const;
    case "configuracoes":
    default:
      return { moduleId: "settings", action: "view" } as const;
  }
}

function getSupplementalProfiles(pathname: string): SystemRole[] {
  if (pathname.startsWith("/admin")) return ADMIN_PROFILES;
  if (pathname.startsWith("/automacoes")) return INTERNAL_PROFILES;
  if (pathname.startsWith("/brain")) return ALL_PROFILES;
  if (pathname.startsWith("/empresas")) return COMPANY_PROFILES;
  return ALL_PROFILES;
}

function getSupplementalStatus(pathname: string): SystemRouteDefinition["status"] {
  if (
    pathname === "/admin" ||
    pathname === "/clients" ||
    pathname === "/clients-list" ||
    pathname === "/docs" ||
    pathname === "/documentacao" ||
    pathname === "/me" ||
    pathname === "/release" ||
    pathname === "/requests" ||
    pathname === "/settings"
  ) {
    return "legado";
  }

  return "parcial";
}

function buildSupplementalRoute(mainFile: string): SystemRouteDefinition {
  const pathWithoutPrefix = mainFile.replace(/^app\//, "").replace(/\/page\.tsx$/, "");
  const pathname = `/${pathWithoutPrefix}`;
  const moduleId = getSupplementalModuleId(pathname);
  const normalizedId = pathWithoutPrefix
    .replace(/\//g, ".")
    .replace(/\[([^\]]+)\]/g, "$1")
    .replace(/[^a-zA-Z0-9._-]/g, "-");

  return {
    id: `suplementar.${normalizedId}`,
    moduleId,
    path: pathname,
    label: formatSupplementalRouteLabel(pathname),
    description: `Tela suplementar mapeada para governança de visibilidade: ${pathname}.`,
    requiredPermission: getSupplementalPermission(moduleId),
    expectedProfiles: getSupplementalProfiles(pathname),
    status: getSupplementalStatus(pathname),
    mainFile,
    notes: "Entrada complementar criada para garantir governança da tela na matriz de perfil e usuário.",
  };
}

const CORE_SYSTEM_ROUTES = [
  {
    id: "home.principal",
    moduleId: "dashboards",
    path: "/home",
    label: "Home",
    description: "Entrada principal adaptada ao perfil autenticado.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/home/page.tsx",
  },
  {
    id: "visao-geral.admin",
    moduleId: "dashboards",
    path: "/admin/visao-geral",
    label: "Visão geral administrativa",
    description: "Resumo operacional e executivo para perfis internos autorizados.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/visao-geral/page.tsx",
  },
  {
    id: "empresas.listagem",
    moduleId: "empresas",
    path: "/admin/clients",
    label: "Listagem de empresas",
    description: "Cadastro e consulta administrativa das empresas.",
    requiredPermission: { moduleId: "applications", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/clients/page.tsx",
  },
  {
    id: "empresas.buscar",
    moduleId: "empresas",
    path: "/admin/clients?focus=search",
    label: "Buscar empresa",
    description: "Abre a listagem administrativa com foco na busca.",
    requiredPermission: { moduleId: "applications", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/clients/page.tsx",
  },
  {
    id: "empresas.criar",
    moduleId: "empresas",
    path: "/admin/clients?modal=create",
    label: "Criar empresa",
    description: "Abre o cadastro de uma empresa.",
    requiredPermission: { moduleId: "applications", action: "create" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/clients/page.tsx",
  },
  {
    id: "empresas.portal",
    moduleId: "empresas",
    path: "/empresas/[slug]/home",
    label: "Portal da empresa",
    description: "Entrada principal do contexto de uma empresa.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: COMPANY_PROFILES,
    status: "ativo",
    mainFile: "app/empresas/[slug]/home/page.tsx",
  },
  {
    id: "empresas.aplicacoes",
    moduleId: "empresas",
    path: "/empresas/[slug]/aplicacoes",
    label: "Aplicações da empresa",
    description: "Aplicações e integrações vinculadas à empresa.",
    requiredPermission: { moduleId: "applications", action: "view" },
    expectedProfiles: COMPANY_PROFILES,
    status: "ativo",
    mainFile: "app/empresas/[slug]/aplicacoes/page.tsx",
  },
  {
    id: "usuarios.listagem",
    moduleId: "usuarios",
    path: "/admin/users",
    label: "Gestão de usuários",
    description: "Listagem, criação e manutenção dos usuários.",
    requiredPermission: { moduleId: "users", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "usuarios.criar-lider",
    moduleId: "usuarios",
    path: "/admin/users?tab=admin&modal=create&role=leader_tc",
    label: "Criar Líder TC",
    description: "Abre o cadastro de um Líder TC.",
    requiredPermission: { moduleId: "users", action: "create" },
    expectedProfiles: [SYSTEM_ROLES.LEADER_TC],
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "usuarios.criar-suporte",
    moduleId: "usuarios",
    path: "/admin/users?tab=support&modal=create&role=technical_support",
    label: "Criar Suporte Técnico",
    description: "Abre o cadastro de um usuário de suporte.",
    requiredPermission: { moduleId: "users", action: "create" },
    expectedProfiles: [SYSTEM_ROLES.LEADER_TC],
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "usuarios.criar-usuário-tc",
    moduleId: "usuarios",
    path: "/admin/users?tab=testing&modal=create&role=testing_company_user",
    label: "Criar Usuários TC",
    description: "Abre o cadastro de um usuário interno.",
    requiredPermission: { moduleId: "users", action: "create" },
    expectedProfiles: [SYSTEM_ROLES.LEADER_TC],
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "usuarios.criar-usuário-empresa",
    moduleId: "usuarios",
    path: "/admin/users?tab=company&modal=create&role=company_user",
    label: "Criar Usuário da Empresa",
    description: "Abre o cadastro de um usuário vinculado à empresa.",
    requiredPermission: { moduleId: "users", action: "create" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "usuarios.criar-usuário",
    moduleId: "usuarios",
    path: "/admin/users?tab=company&modal=create&role=company_user",
    label: "Criar usuário",
    description: "Abre o cadastro de um usuário da própria empresa.",
    requiredPermission: { moduleId: "users", action: "create" },
    expectedProfiles: [SYSTEM_ROLES.EMPRESA],
    status: "ativo",
    mainFile: "app/admin/users/page.tsx",
  },
  {
    id: "permissoes.perfil",
    moduleId: "permissoes",
    path: "/admin/permissions",
    label: "Gestão de perfil",
    description: "Gestão das permissões padrão por perfil.",
    requiredPermission: { moduleId: "permissions", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/permissions/page.tsx",
  },
  {
    id: "permissoes.matriz",
    moduleId: "permissoes",
    path: "/admin/users/permissions",
    label: "Gestão de usuário",
    description: "Perfil base e permissões efetivas por usuário.",
    requiredPermission: { moduleId: "permissions", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/users/permissions/page.tsx",
  },
  {
    id: "permissoes.atalho-admin",
    moduleId: "permissoes",
    path: "/admin/permissões",
    label: "Permissões administrativas",
    description: "Entrada administrativa anterior para permissões.",
    requiredPermission: { moduleId: "permissions", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "legado",
    mainFile: "app/admin/permissoes/page.tsx",
    notes: "Manter enquanto houver links apontando para esta rota; consolidar com a matriz principal.",
  },
  {
    id: "testes-manuais.casos",
    moduleId: "testes-manuais",
    path: "/casos-de-teste",
    label: "Casos de teste",
    description: "Repositório central de casos de teste.",
    requiredPermission: { moduleId: "test_repository", action: "read" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/casos-de-teste/page.tsx",
  },
  {
    id: "testes-manuais.planos",
    moduleId: "testes-manuais",
    path: "/empresas/[slug]/planos-de-teste",
    label: "Planos de teste",
    description: "Planejamento de ciclos de teste por empresa.",
    requiredPermission: { moduleId: "test_plan", action: "read" },
    expectedProfiles: COMPANY_PROFILES,
    status: "ativo",
    mainFile: "app/empresas/[slug]/planos-de-teste/page.tsx",
  },
  {
    id: "testes-manuais.runs",
    moduleId: "testes-manuais",
    path: "/empresas/[slug]/runs",
    label: "Runs",
    description: "Execuções de teste no contexto da empresa.",
    requiredPermission: { moduleId: "test_run", action: "read" },
    expectedProfiles: COMPANY_PROFILES,
    status: "ativo",
    mainFile: "app/empresas/[slug]/runs/page.tsx",
  },
  {
    id: "execucoes.runs",
    moduleId: "testes-manuais",
    path: "/runs",
    label: "Execuções",
    description: "Entrada global de execuções, respeitando escopo de empresa e permissões efetivas.",
    requiredPermission: { moduleId: "test_run", action: "read" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/runs/page.tsx",
  },
  {
    id: "testes-manuais.defeitos",
    moduleId: "testes-manuais",
    path: "/empresas/[slug]/defeitos",
    label: "Defeitos",
    description: "Registro e acompanhamento de defeitos da empresa.",
    requiredPermission: { moduleId: "defect_tracking", action: "read" },
    expectedProfiles: COMPANY_PROFILES,
    status: "parcial",
    mainFile: "app/empresas/[slug]/defeitos/page.tsx",
    notes: "Tela extensa e candidata a divisão incremental em componentes.",
  },
  {
    id: "defeitos.lista",
    moduleId: "testes-manuais",
    path: "/defeitos",
    label: "Defeitos",
    description: "Entrada global de defeitos, redirecionada para a área administrativa conforme contexto.",
    requiredPermission: { moduleId: "defect_tracking", action: "read" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/defeitos/page.tsx",
  },
  {
    id: "testes-manuais.releases",
    moduleId: "testes-manuais",
    path: "/empresas/[slug]/releases",
    label: "Releases",
    description: "Releases e decisão de qualidade por empresa.",
    requiredPermission: { moduleId: "release_management", action: "read" },
    expectedProfiles: COMPANY_PROFILES,
    status: "ativo",
    mainFile: "app/empresas/[slug]/releases/page.tsx",
  },
  {
    id: "automacao.inicio",
    moduleId: "automacao",
    path: "/automacoes",
    label: "Central de automação",
    description: "Entrada do ambiente de automação.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/page.tsx",
  },
  {
    id: "automacao.playwright",
    moduleId: "automacao",
    path: "/automacoes/playwright",
    label: "Playwright",
    description: "Criação e execução de automações Playwright.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/playwright/page.tsx",
  },
  {
    id: "automacao.studio",
    moduleId: "automacao",
    path: "/automacoes/ui-studio",
    label: "UI Studio",
    description: "Ambiente visual para montagem de automações.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "parcial",
    mainFile: "app/automacoes/ui-studio/page.tsx",
    notes: "A implementação principal ainda está concentrada em AutomationStudio.tsx.",
  },
  {
    id: "automacao.execucoes",
    moduleId: "automacao",
    path: "/automacoes/execucoes",
    label: "Execuções automatizadas",
    description: "Histórico e estado das execuções automatizadas.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/execucoes/page.tsx",
  },
  {
    id: "automacao.fluxos",
    moduleId: "automacao",
    path: "/automacoes/fluxos",
    label: "Fluxos automatizados",
    description: "Alias legado: redireciona para o UI Studio (modo Fluxo).",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "legado",
    mainFile: "app/automacoes/fluxos/page.tsx",
    notes: "Fluxos, Scripts e UI Studio eram 3 rotas para o mesmo componente (AutomationStudio.tsx). A tela canonica passou a ser /automacoes/ui-studio; esta rota so redireciona.",
  },
  {
    id: "automacao.casos",
    moduleId: "automacao",
    path: "/automacoes/casos",
    label: "Casos automatizados",
    description: "Casos associados a automações.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "legado",
    mainFile: "app/automacoes/casos/page.tsx",
    notes: "A navegação canônica de casos manuais permanece em /casos-de-teste.",
  },
  {
    id: "automacao.scripts",
    moduleId: "automacao",
    path: "/automacoes/scripts",
    label: "Scripts",
    description: "Alias legado: redireciona para o UI Studio (modo Script).",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "legado",
    mainFile: "app/automacoes/scripts/page.tsx",
    notes: "Fluxos, Scripts e UI Studio eram 3 rotas para o mesmo componente (AutomationStudio.tsx). A tela canonica passou a ser /automacoes/ui-studio; esta rota so redireciona.",
  },
  {
    id: "automacao.ferramentas",
    moduleId: "automacao",
    path: "/automacoes/tools",
    label: "Ferramentas",
    description: "Ferramentas auxiliares de automação.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/tools/page.tsx",
  },
  {
    id: "automacao.api-lab",
    moduleId: "automacao",
    path: "/automacoes/api-lab",
    label: "API Lab",
    description: "Ambiente para testar chamadas HTTP.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/api-lab/page.tsx",
  },
  {
    id: "automacao.base64",
    moduleId: "automacao",
    path: "/automacoes/base64",
    label: "Base64 e encoders",
    description: "Ferramentas para codificação e transformação.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/base64/page.tsx",
  },
  {
    id: "automacao.biblioteca",
    moduleId: "automacao",
    path: "/automacoes/base64?tab=library",
    label: "Biblioteca de arquivos",
    description: "Abre a biblioteca de ativos da automação.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "parcial",
    mainFile: "app/automacoes/base64/page.tsx",
  },
  {
    id: "automacao.logs",
    moduleId: "automacao",
    path: "/automacoes/logs",
    label: "Logs de automação",
    description: "Consulta dos registros das automações.",
    requiredPermission: { moduleId: "playwright", action: "read" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/automacoes/logs/page.tsx",
  },
  {
    id: "brain.grafo",
    moduleId: "brain",
    path: "/brain",
    label: "Grafo do Brain",
    description: "Visualização do grafo de conhecimento.",
    requiredPermission: { moduleId: "brain", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/brain/page.tsx",
  },
  {
    id: "brain.admin",
    moduleId: "brain",
    path: "/admin/brain",
    label: "Mapa Neural",
    description: "Mapa neural administrativo do Brain, usando a tela atual de nós e relações.",
    requiredPermission: { moduleId: "brain", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/brain/page.tsx",
  },
  {
    id: "brain.memories",
    moduleId: "brain",
    path: "/admin/brain/memories",
    label: "Memórias do Brain",
    description: "Lista, filtra e gerencia memorias e contexto RAG do Brain.",
    requiredPermission: { moduleId: "brain", action: "manage_memories" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/brain/memories/page.tsx",
  },
  {
    id: "brain.settings.admin",
    moduleId: "brain",
    path: "/admin/brain/settings",
    label: "Configurações do Brain",
    description: "Configura fontes, APIs, bancos, sites, web controlada e escopos do Brain.",
    requiredPermission: { moduleId: "brain", action: "configure_sources" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/brain/settings/page.tsx",
  },
  {
    id: "brain.settings.company",
    moduleId: "brain",
    path: "/brain/settings",
    label: "Configurações do Brain",
    description: "Configura fontes do Brain no escopo permitido da empresa.",
    requiredPermission: { moduleId: "brain", action: "configure_sources" },
    expectedProfiles: [SYSTEM_ROLES.EMPRESA, SYSTEM_ROLES.COMPANY_USER],
    status: "ativo",
    mainFile: "app/brain/settings/page.tsx",
  },
  {
    id: "assistente.perguntar",
    moduleId: "assistente",
    path: "/brain/perguntar",
    label: "Perguntar ao assistente",
    description: "Conversa orientada pelo contexto do produto.",
    requiredPermission: { moduleId: "ai", action: "use" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/brain/perguntar/page.tsx",
  },
  {
    id: "chat.principal",
    moduleId: "chat",
    path: "/chat",
    label: "Chat",
    description: "Lista de conversas e troca de mensagens.",
    requiredPermission: { moduleId: "chat", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "parcial",
    mainFile: "app/chat/page.tsx",
  },
  {
    id: "chat.buscar",
    moduleId: "chat",
    path: "/chat?focus=search",
    label: "Buscar conversa",
    description: "Rota legada removida do menu. A busca fica dentro do Chat.",
    requiredPermission: { moduleId: "chat", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "legado",
    mainFile: "app/chat/page.tsx",
  },
  {
    id: "chat.conversas",
    moduleId: "chat",
    path: "/conversas",
    label: "Conversas",
    description: "Entrada alternativa para conversas.",
    requiredPermission: { moduleId: "chat", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "legado",
    mainFile: "app/conversas/page.tsx",
    notes: "Avaliar consolidação com /chat.",
  },
  {
    id: "agenda.meus-agendamentos",
    moduleId: "agenda",
    path: "/agenda?view=mine",
    label: "Meus agendamentos",
    description: "Agenda filtrada para o usuário autenticado e seus contextos permitidos.",
    requiredPermission: { moduleId: "release_calendar", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/agenda/page.tsx",
  },
  {
    id: "agenda.agendamentos-gerais",
    moduleId: "agenda",
    path: "/agenda?view=management",
    label: "Agendamentos gerais",
    description: "Calendário operacional para perfis internos com permissão de agenda.",
    requiredPermission: { moduleId: "release_calendar", action: "view" },
    expectedProfiles: INTERNAL_PROFILES,
    status: "ativo",
    mainFile: "app/agenda/page.tsx",
  },
  {
    id: "suporte.central",
    moduleId: "suporte",
    path: "/suporte",
    label: "Central de suporte",
    description: "Abertura e consulta de chamados.",
    requiredPermission: { moduleId: "support", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/suporte/page.tsx",
  },
  {
    id: "suporte.criar",
    moduleId: "suporte",
    path: "/suporte?modal=create",
    label: "Criar chamado",
    description: "Abre o formulário de novo chamado.",
    requiredPermission: { moduleId: "support", action: "create" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/suporte/page.tsx",
  },
  {
    id: "suporte.kanban",
    moduleId: "suporte",
    path: "/suporte/kanban",
    label: "Kanban de chamados",
    description: "Andamento operacional dos chamados.",
    requiredPermission: { moduleId: "support", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/suporte/kanban/page.tsx",
  },
  {
    id: "suporte.chamados",
    moduleId: "suporte",
    path: "/chamados",
    label: "Chamados",
    description: "Visão ampla de chamados para perfis internos.",
    requiredPermission: { moduleId: "tickets", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/chamados/page.tsx",
  },
  {
    id: "suporte.meus-chamados",
    moduleId: "suporte",
    path: "/meus-chamados",
    label: "Meus chamados",
    description: "Chamados visíveis para o usuário autenticado.",
    requiredPermission: { moduleId: "tickets", action: "view_own" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/meus-chamados/page.tsx",
  },
  {
    id: "solicitacoes.listagem",
    moduleId: "solicitacoes",
    path: "/solicitacoes",
    label: "Solicitações",
    description: "Fila administrativa de solicitações.",
    requiredPermission: { moduleId: "access_requests", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/solicitacoes/page.tsx",
  },
  {
    id: "solicitacoes.buscar",
    moduleId: "solicitacoes",
    path: "/solicitacoes?focus=search",
    label: "Buscar solicitação",
    description: "Abre a fila de solicitações com foco na busca.",
    requiredPermission: { moduleId: "access_requests", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/solicitacoes/page.tsx",
  },
  {
    id: "solicitacoes.acessos-admin",
    moduleId: "solicitacoes",
    path: "/admin/access-requests",
    label: "Solicitações de acesso",
    description: "Fluxo administrativo detalhado de acessos.",
    requiredPermission: { moduleId: "access_requests", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "parcial",
    mainFile: "app/admin/access-requests/page.tsx",
    notes: "Convive com /solicitacoes e deve ter responsabilidade consolidada futuramente.",
  },
  {
    id: "agenda.release",
    moduleId: "agenda",
    path: "/agenda",
    label: "Agenda",
    description: "Calendário de releases e compromissos operacionais.",
    requiredPermission: { moduleId: "release_calendar", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/agenda/page.tsx",
  },
  {
    id: "documentos.central",
    moduleId: "documentos",
    path: "/documentos",
    label: "Central de documentos",
    description: "Acesso principal aos documentos.",
    requiredPermission: { moduleId: "documents", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/documentos/page.tsx",
  },
  {
    id: "documentos.repositorio",
    moduleId: "documentos",
    path: "/documentos/repositorio",
    label: "Repositório de documentos",
    description: "Organização e consulta do acervo.",
    requiredPermission: { moduleId: "documents", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/documentos/repositorio/page.tsx",
  },
  {
    id: "documentos.plataforma",
    moduleId: "documentos",
    path: "/docs/platform",
    label: "Documentação da plataforma",
    description: "Documentação técnica e funcional da plataforma.",
    requiredPermission: { moduleId: "documents", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "oculto",
    mainFile: "app/docs/platform/page.tsx",
    notes: "Rota existente fora do catálogo principal de documentos.",
  },
  {
    id: "dashboards.geral",
    moduleId: "dashboards",
    path: "/dashboard",
    label: "Visão Geral",
    description: "Visão geral executiva ou operacional conforme o perfil.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/dashboard/page.tsx",
  },
  {
    id: "dashboards.admin",
    moduleId: "dashboards",
    path: "/admin/dashboard",
    label: "Dashboard administrativo",
    description: "Indicadores globais e ranking de empresas.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/dashboard/page.tsx",
  },
  {
    id: "dashboards.empresa",
    moduleId: "dashboards",
    path: "/empresas/[slug]/dashboard",
    label: "Central de Qualidade da Empresa",
    description: "Indicadores de qualidade no contexto da empresa.",
    requiredPermission: { moduleId: "dashboard", action: "view" },
    expectedProfiles: COMPANY_PROFILES,
    status: "parcial",
    mainFile: "app/empresas/[slug]/dashboard/page.tsx",
    notes: "A implementação cliente é extensa e candidata a divisão incremental.",
  },
  {
    id: "operacao.inicio",
    moduleId: "operacao",
    path: "/operacoes",
    label: "Operações",
    description: "Entrada da área operacional.",
    requiredPermission: { moduleId: "operations", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/operacoes/page.tsx",
  },
  {
    id: "operacao.dashboard",
    moduleId: "operacao",
    path: "/operacoes/dashboard",
    label: "Dashboard operacional",
    description: "Resumo operacional do ambiente.",
    requiredPermission: { moduleId: "operations", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/operacoes/dashboard/page.tsx",
  },
  {
    id: "operacao.metricas",
    moduleId: "operacao",
    path: "/operacoes/metricas",
    label: "Métricas operacionais",
    description: "Métricas consolidadas da operação.",
    requiredPermission: { moduleId: "operations", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "ativo",
    mainFile: "app/operacoes/metricas/page.tsx",
  },
  {
    id: "operacao.busca",
    moduleId: "operacao",
    path: "/operacoes/buscar",
    label: "Busca operacional",
    description: "Busca transversal por empresas e itens operacionais.",
    requiredPermission: { moduleId: "operations", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/operacoes/buscar/page.tsx",
  },
  {
    id: "configuracoes.perfil",
    moduleId: "configuracoes",
    path: "/settings/profile",
    label: "Perfil",
    description: "Dados pessoais, preferências e segurança.",
    requiredPermission: { moduleId: "settings", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "parcial",
    mainFile: "app/settings/profile/page.tsx",
    notes: "Tela extensa e candidata a divisão incremental por seção.",
  },
  {
    id: "gestao.perfil",
    moduleId: "permissoes",
    path: "/admin/profile",
    label: "Perfil de gestão",
    description: "Entrada legada da gestão que redireciona para a matriz de permissões.",
    requiredPermission: { moduleId: "permissions", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "legado",
    mainFile: "app/admin/profile/page.tsx",
  },
  {
    id: "configuracoes.admin",
    moduleId: "configuracoes",
    path: "/settings/admin",
    label: "Configurações administrativas",
    description: "Configurações reservadas aos administradores.",
    requiredPermission: { moduleId: "settings", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "oculto",
    mainFile: "app/settings/admin/page.tsx",
  },
  {
    id: "configuracoes.auditoria",
    moduleId: "configuracoes",
    path: "/audit-logs?source=admin",
    label: "Logs de auditoria",
    description: "Eventos administrativos e alterações sensíveis.",
    requiredPermission: { moduleId: "audit", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/audit-logs/page.tsx",
  },
  {
    id: "logs.sistema",
    moduleId: "configuracoes",
    path: "/admin/logs",
    label: "Logs do sistema",
    description: "Logs operacionais e técnicos visíveis para perfis internos autorizados.",
    requiredPermission: { moduleId: "audit", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/logs/page.tsx",
  },
  {
    id: "configuracoes.mapa-sistema",
    moduleId: "configuracoes",
    path: "/admin/sistema/mapa",
    label: "Mapa do Sistema",
    description: "Governança de módulos, rotas e permissões.",
    requiredPermission: { moduleId: "permissions", action: "view" },
    expectedProfiles: ADMIN_PROFILES,
    status: "ativo",
    mainFile: "app/admin/sistema/mapa/page.tsx",
  },
  {
    id: "configuracoes.perfil-legado",
    moduleId: "configuracoes",
    path: "/profile",
    label: "Perfil legado",
    description: "Entrada anterior para dados do perfil.",
    requiredPermission: { moduleId: "settings", action: "view" },
    expectedProfiles: ALL_PROFILES,
    status: "legado",
    mainFile: "app/profile/page.tsx",
    notes: "Avaliar consolidação com /settings/profile.",
  },
] as const satisfies readonly SystemRouteDefinition[];

const SUPPLEMENTAL_SYSTEM_ROUTES = [
  "app/admin/alerts/page.tsx",
  "app/admin/audit-logs/page.tsx",
  "app/admin/chamados/page.tsx",
  "app/admin/defeitos/page.tsx",
  "app/admin/docs/page.tsx",
  "app/admin/home/page.tsx",
  "app/admin/page.tsx",
  "app/admin/releases/page.tsx",
  "app/admin/requests/page.tsx",
  "app/admin/runs/page.tsx",
  "app/admin/support/page.tsx",
  "app/admin/test-metric/page.tsx",
  "app/agenda/calendario-geral/page.tsx",
  "app/agenda/meus-agendamentos/page.tsx",
  "app/agenda/novo/page.tsx",
  "app/applications-hub/page.tsx",
  "app/applications-panel/page.tsx",
  "app/automacoes/arquivos/page.tsx",
  "app/automacoes/ferramentas/page.tsx",
  "app/automacoes/quality-control/page.tsx",
  "app/brain/acoes/page.tsx",
  "app/brain/prompts/page.tsx",
  "app/brain/qa/page.tsx",
  "app/brain/solicitacoes/page.tsx",
  "app/brain/validacoes/page.tsx",
  "app/brand-identity/page.tsx",
  "app/central-de-qualidade/page.tsx",
  "app/clients-list/page.tsx",
  "app/clients/page.tsx",
  "app/dashboard/apps/page.tsx",
  "app/docs/page.tsx",
  "app/documentacao/page.tsx",
  "app/empresas/[slug]/admin/page.tsx",
  "app/empresas/[slug]/casos-de-teste/page.tsx",
  "app/empresas/[slug]/chamados/page.tsx",
  "app/empresas/[slug]/defeitos/kanban/page.tsx",
  "app/empresas/[slug]/docs/page.tsx",
  "app/empresas/[slug]/documentos/page.tsx",
  "app/empresas/[slug]/jira/page.tsx",
  "app/empresas/[slug]/metrics/page.tsx",
  "app/empresas/[slug]/page.tsx",
  "app/empresas/[slug]/perfil/page.tsx",
  "app/empresas/[slug]/profile/page.tsx",
  "app/empresas/[slug]/projetos/page.tsx",
  "app/empresas/[slug]/releases/[releaseSlug]/page.tsx",
  "app/empresas/[slug]/runs/[runSlug]/page.tsx",
  "app/empresas/page.tsx",
  "app/issues/page.tsx",
  "app/kanban-it/page.tsx",
  "app/me/page.tsx",
  "app/metrics/page.tsx",
  "app/modelo-qualidade/page.tsx",
  "app/notificacoes/page.tsx",
  "app/painel-releases-manuais-autenticado/page.tsx",
  "app/painel-releases-manuais/page.tsx",
  "app/release/[slug]/page.tsx",
  "app/release/page.tsx",
  "app/requests/page.tsx",
  "app/runs/[id]/page.tsx",
  "app/settings/page.tsx",
] as const satisfies readonly string[];

export const SYSTEM_ROUTES = [
  ...CORE_SYSTEM_ROUTES,
  ...SUPPLEMENTAL_SYSTEM_ROUTES.map((mainFile) => buildSupplementalRoute(mainFile)),
] as const satisfies readonly SystemRouteDefinition[];

export const SYSTEM_ROUTE_BY_ID: ReadonlyMap<string, SystemRouteDefinition> = new Map(
  SYSTEM_ROUTES.map((routeDefinition) => [routeDefinition.id, routeDefinition]),
);
