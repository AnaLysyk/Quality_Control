    ...
    <main className="space-y-5">
      <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-sm leading-6 text-(--tc-text-secondary,#4b5563)">Eventos críticos não podem ser desativados; eventos operacionais podem ser suprimidos, mas continuam auditáveis e visíveis para o Brain.</p>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <div className="space-y-4">
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
            <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiSliders className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Configurar recebimento</div>
            <p className="mt-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">Use para desligar ou religar um canal por empresa, perfil ou usuário. A lista abaixo já inclui agenda, tickets, docs, acesso e segurança.</p>
            <form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={handlePreferenceSubmit}>
              ...
              <div className="space-y-1"><FieldLabel>Alvo</FieldLabel><select value={preferenceForm.target} onChange={(event) => setPreferenceForm((current) => ({ ...current, target: event.target.value as PreferenceFormState["target"] }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold text-(--tc-text,#0b1a3c)"><option value="company">Empresa</option><option value="profile">Perfil</option><option value="user">Usuário</option></select></div>
              <div className="space-y-1"><FieldLabel>ID do alvo</FieldLabel><input value={preferenceForm.targetId} onChange={(event) => setPreferenceForm((current) => ({ ...current, targetId: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold text-(--tc-text,#0b1a3c)" placeholder="testing-company / qa-lead / user-id" /></div>
              <div className="space-y-1"><FieldLabel>Workflow</FieldLabel><select value={preferenceForm.workflowId} onChange={(event) => setPreferenceForm((current) => ({ ...current, workflowId: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold text-(--tc-text,#0b1a3c)">{(data?.workflows ?? []).map((workflow) => <option key={workflow.id} value={workflow.id}>{workflow.label} · {workflow.id}</option>)}</select></div>
              <div className="space-y-1"><FieldLabel>Canal</FieldLabel><select value={preferenceForm.channel} onChange={(event) => setPreferenceForm((current) => ({ ...current, channel: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold text-(--tc-text,#0b1a3c)">{(data?.channels ?? ["in_app", "email", "push", "chat", "brain"]).map((channel) => <option key={channel} value={channel}>{channel}</option>)}</select></div>
              <div className="space-y-1"><FieldLabel>Decisão</FieldLabel><select value={preferenceForm.decision} onChange={(event) => setPreferenceForm((current) => ({ ...current, decision: event.target.value as PreferenceFormState["decision"] }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold text-(--tc-text,#0b1a3c)"><option value="disabled">Desativar recebimento</option><option value="enabled">Ativar recebimento</option></select></div>
              <div className="flex items-end"><button type="submit" disabled={savingPreference} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-(--tc-accent,#ef0001) px-4 py-2 text-sm font-black text-white disabled:opacity-60"><FiSave className="h-4 w-4" /> {savingPreference ? "Salvando..." : "Salvar preferência"}</button></div>
            </form>
          </section>
        ...
        <div className="space-y-4">
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiUsers className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Preferências cadastradas</div><div className="mt-4 grid gap-3 sm:grid-cols-3"><StatCard label="Empresa" value={data?.preferenceSummary.company ?? 0} note="Regras por empresa" /><StatCard label="Perfil" value={data?.preferenceSummary.profile ?? 0} note="Regras por perfil" /><StatCard label="Usuário" value={data?.preferenceSummary.user ?? 0} note="Regras individuais" /></div><div className="mt-4 space-y-2">{data?.preferences?.length ? data.preferences.map((preference) => <article key={preference.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-3"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-black text-(--tc-text,#0b1a3c)">{preference.target}: {preference.targetId}</h2><div className="flex flex-wrap gap-2"><Badge>{preference.workflowId}</Badge><Badge>{preference.channel}</Badge><Badge>{preference.decision}</Badge></div></div><p className="mt-1 text-xs font-semibold text-(--tc-text-muted,#6b7280)">Atualizado em {formatDate(preference.updatedAt)}</p></article>) : <p className="text-sm font-semibold text-(--tc-text-muted,#6b7280)">Nenhuma preferência cadastrada ainda.</p>}</div></section>
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiLayers className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Workflows de notificação</div><div className="mt-4 space-y-3">{data?.workflows.map((workflow) => <article key={workflow.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{workflow.eventType} · {workflow.category}</p><h2 className="mt-1 text-base font-black text-(--tc-text,#0b1a3c)">{workflow.label}</h2></div><div className="flex flex-wrap gap-2"><Badge>{workflow.criticality}</Badge><Badge>{workflow.mandatory ? "Obrigatório" : "Configurável"}</Badge></div></div><p className="mt-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{workflow.description}</p><div className="mt-3 flex flex-wrap gap-2">{workflow.defaultChannels.map((channel) => <Badge key={channel}>{channel}</Badge>)}</div></article>)}</div></section>
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiSliders className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Prioridade das preferências</div><div className="mt-4 space-y-3">{data?.preferenceLayers.map((layer) => <article key={layer.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-black text-(--tc-text,#0b1a3c)">{layer.priority}. {layer.label}</h2><FiShield className="h-4 w-4 text-(--tc-accent,#ef0001)" /></div><p className="mt-2 text-xs leading-5 text-(--tc-text-secondary,#4b5563)">{layer.description}</p></article>)}</div></section>
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiCheckCircle className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Regras duras</div><ul className="mt-4 space-y-2 text-sm leading-6 text-(--tc-text-secondary,#4b5563)">{data?.rules.map((rule) => <li key={rule}>• {rule}</li>)}</ul></section>
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)"><FiCpu className="h-4 w-4 text-(--tc-accent,#ef0001)" /> Backlog de implementação</div><div className="mt-4 space-y-3">{data?.backlog.map((item) => <article key={item.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-black text-(--tc-text,#0b1a3c)">{item.title}</h2><div className="flex gap-2"><Badge>{item.area}</Badge><Badge>{item.priority}</Badge></div></div><ul className="mt-3 space-y-1 text-xs leading-5 text-(--tc-text-secondary,#4b5563)">{item.acceptanceCriteria.map((criteria) => <li key={criteria}>• {criteria}</li>)}</ul></article>)}</div></section>
        </div>
      </section>
    </main>
  );
