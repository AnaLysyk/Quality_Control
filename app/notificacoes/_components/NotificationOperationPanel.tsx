...
function shortPayload(payload: Record<string, unknown>) {
  ...
  return [releaseName, releaseId, calendarStatus, notificationType].filter(Boolean).join(" Â· ");
}
...
function operationalFacts(event?: NotificationEventRecord | null) {
  ...
  return [
    area ? { label: "Ãrea", value: area } : null,
    entity ? { label: "Entidade", value: entity } : null,
    action ? { label: "AÃ§Ã£o", value: action } : null,
    ...
    <main className="space-y-5">
      <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]">
              <FiBell className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> Central de notificaÃ§Ãµes
            </span>
            <h1 className="mt-3 text-2xl font-black text-[var(--tc-text,#0b1a3c)]">Evento sempre registrado, recebimento configurÃ¡vel</h1>
            <p className="mt-2 max-w-4xl text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">Eventos crÃ­ticos nÃ£o podem ser desativados; eventos operacionais podem ser suprimidos, mas continuam auditÃ¡veis e visÃ­veis para o Brain.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-6 xl:grid-cols-10">
          <StatCard label="Workflows" value={isLoading ? "..." : data?.summary.workflows ?? 0} note="CatÃ¡logo" />
          <StatCard label="ObrigatÃ³rios" value={isLoading ? "..." : data?.summary.mandatory ?? 0} note="Sem opt-out" />
          <StatCard label="Canais" value={isLoading ? "..." : data?.summary.channels ?? 0} note="Entrega" />
          <StatCard label="PreferÃªncias" value={isLoading ? "..." : data?.preferenceSummary.total ?? 0} note="Store" />
          <StatCard label="Desativadas" value={isLoading ? "..." : data?.preferenceSummary.disabled ?? 0} note="SupressÃ£o" />
          <StatCard label="Eventos" value={isLoading ? "..." : data?.notificationEventsSummary.events ?? 0} note="AuditÃ¡veis" />
          ...
          <StatCard label="Brian" value={isLoading ? "..." : data?.notificationBrianInsights?.length ?? 0} note="ExplicaÃ§Ãµes" />
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        <div className="space-y-4">
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">Use para desligar ou religar um canal por empresa, perfil ou usuÃ¡rio. A lista abaixo jÃ¡ inclui agenda, tickets, docs, acesso e seguranÃ§a.</p>
            <form className="mt-4 grid gap-3 md:grid-cols-2" onSubmit={handlePreferenceSubmit}>
              ...
              <div className="space-y-1"><FieldLabel>DecisÃ£o</FieldLabel><select value={preferenceForm.decision} onChange={(event) => setPreferenceForm((current) => ({ ...current, decision: event.target.value as PreferenceFormState["decision"] }))} className="w-full rounded-xl border border-[var(--tc-border,#d7deea)] bg-white px-3 py-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]"><option value="disabled">Desativar recebimento</option><option value="enabled">Ativar recebimento</option></select></div>
              <div className="flex items-end"><button type="submit" disabled={savingPreference} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--tc-accent,#ef0001)] px-4 py-2 text-sm font-black text-white disabled:opacity-60"><FiSave className="h-4 w-4" /> {savingPreference ? "Salvando..." : "Salvar preferÃªncia"}</button></div>
            </form>
          </section>
        ...
        <div className="space-y-4">
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]"><FiUsers className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> PreferÃªncias cadastradas</div><div className="mt-4 grid gap-3 sm:grid-cols-3"><StatCard label="Empresa" value={data?.preferenceSummary.company ?? 0} note="Regras por empresa" /><StatCard label="Perfil" value={data?.preferenceSummary.profile ?? 0} note="Regras por perfil" /><StatCard label="UsuÃ¡rio" value={data?.preferenceSummary.user ?? 0} note="Regras individuais" /></div><div className="mt-4 space-y-2">{data?.preferences?.length ? data.preferences.map((preference) => <article key={preference.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">{preference.target}: {preference.targetId}</h2><div className="flex flex-wrap gap-2"><Badge>{preference.workflowId}</Badge><Badge>{preference.channel}</Badge><Badge>{preference.decision}</Badge></div></div><p className="mt-1 text-xs font-semibold text-[var(--tc-text-muted,#6b7280)]">Atualizado em {formatDate(preference.updatedAt)}</p></article>) : <p className="text-sm font-semibold text-[var(--tc-text-muted,#6b7280)]">Nenhuma preferÃªncia cadastrada ainda.</p>}</div></section>
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]"><FiLayers className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> Workflows de notificaÃ§Ã£o</div><div className="mt-4 space-y-3">{data?.workflows.map((workflow) => <article key={workflow.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]">{workflow.eventType} Â· {workflow.category}</p><h2 className="mt-1 text-base font-black text-[var(--tc-text,#0b1a3c)]">{workflow.label}</h2></div><div className="flex flex-wrap gap-2"><Badge>{workflow.criticality}</Badge><Badge>{workflow.mandatory ? "ObrigatÃ³rio" : "ConfigurÃ¡vel"}</Badge></div></div><p className="mt-2 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">{workflow.description}</p><div className="mt-3 flex flex-wrap gap-2">{workflow.defaultChannels.map((channel) => <Badge key={channel}>{channel}</Badge>)}</div></article>)}</div></section>
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]"><FiSliders className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> Prioridade das preferÃªncias</div><div className="mt-4 space-y-3">{data?.preferenceLayers.map((layer) => <article key={layer.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4"><div className="flex items-center justify-between gap-3"><h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">{layer.priority}. {layer.label}</h2><FiShield className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /></div><p className="mt-2 text-xs leading-5 text-[var(--tc-text-secondary,#4b5563)]">{layer.description}</p></article>)}</div></section>
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]"><FiCheckCircle className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> Regras duras</div><ul className="mt-4 space-y-2 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">{data?.rules.map((rule) => <li key={rule}>â€¢ {rule}</li>)}</ul></section>
          <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm"><div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]"><FiCpu className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /> Backlog de implementaÃ§Ã£o</div><div className="mt-4 space-y-3">{data?.backlog.map((item) => <article key={item.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">{item.title}</h2><div className="flex gap-2"><Badge>{item.area}</Badge><Badge>{item.priority}</Badge></div></div><ul className="mt-3 space-y-1 text-xs leading-5 text-[var(--tc-text-secondary,#4b5563)]">{item.acceptanceCriteria.map((criteria) => <li key={criteria}>â€¢ {criteria}</li>)}</ul></article>)}</div></section>
        </div>
      </section>
    </main>
  );
