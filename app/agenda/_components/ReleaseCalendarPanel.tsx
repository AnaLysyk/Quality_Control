    ...
    <main className="space-y-5">
      ...
      <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-8">
          <div className="space-y-1"><FieldLabel>Empresa</FieldLabel><input list="release-calendar-companies" value={filters.companySlug} onChange={(event) => setFilters((current) => ({ ...current, companySlug: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="testing-company" /></div>
          <div className="space-y-1"><FieldLabel>Projeto</FieldLabel><input list="release-calendar-projects" value={filters.projectSlug} onChange={(event) => setFilters((current) => ({ ...current, projectSlug: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="cid" /></div>
          <div className="space-y-1"><FieldLabel>Usuario</FieldLabel><input list="release-calendar-owners" value={filters.ownerName} onChange={(event) => setFilters((current) => ({ ...current, ownerName: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="QA / suporte" /></div>
          <div className="space-y-1"><FieldLabel>Release</FieldLabel><input list="release-calendar-releases" value={filters.releaseId} onChange={(event) => setFilters((current) => ({ ...current, releaseId: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="cid-2-3-0" /></div>
          <div className="space-y-1"><FieldLabel>Contexto</FieldLabel><select aria-label="Filtrar por contexto" value={filters.context} onChange={(event) => setFilters((current) => ({ ...current, context: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold"><option value="">Todos</option>{CONTEXT_OPTIONS.map((context) => <option key={context} value={context}>{contextLabel(context)}</option>)}</select></div>
          <div className="space-y-1"><FieldLabel>Perfil</FieldLabel><select aria-label="Filtrar por perfil" value={filters.audienceProfile} onChange={(event) => setFilters((current) => ({ ...current, audienceProfile: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold"><option value="">Todos</option>{AUDIENCE_PROFILE_OPTIONS.map((profile) => <option key={profile} value={profile}>{audienceLabel(profile)}</option>)}</select></div>
          <div className="space-y-1"><FieldLabel>Status</FieldLabel><select aria-label="Filtrar por status" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold"><option value="">Todos</option>{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></div>
          <div className="flex items-end"><button type="button" onClick={() => setFilters(initialFilters)} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-black">Limpar</button></div>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
            ...
            <form className="mt-4 space-y-4" onSubmit={handleCreateEvent}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="space-y-1 md:col-span-2"><FieldLabel>Titulo</FieldLabel><input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Janela de regressao CID 2.3.0" /></div>
                <div className="space-y-1"><FieldLabel>Marcacao curta</FieldLabel><input value={form.markerLabel} onChange={(event) => setForm((current) => ({ ...current, markerLabel: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Regressao CID" /></div>
                <div className="space-y-1"><FieldLabel>Tipo</FieldLabel><select aria-label="Tipo do evento" value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold">{EVENT_TYPE_OPTIONS.map((type) => <option key={type} value={type}>{type}</option>)}</select></div>
                <div className="space-y-1"><FieldLabel>Status</FieldLabel><select aria-label="Status do evento" value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></div>
                ...
                <div className="space-y-1"><FieldLabel>Contexto</FieldLabel><select aria-label="Contexto do evento" value={form.context} onChange={(event) => setForm((current) => ({ ...current, context: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold">{CONTEXT_OPTIONS.map((context) => <option key={context} value={context}>{contextLabel(context)}</option>)}</select></div>
                <div className="space-y-1"><FieldLabel>Release ID</FieldLabel><input value={form.releaseId} onChange={(event) => setForm((current) => ({ ...current, releaseId: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="cid-2-3-0" /></div>
                <div className="space-y-1"><FieldLabel>Nome da release</FieldLabel><input value={form.releaseName} onChange={(event) => setForm((current) => ({ ...current, releaseName: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Cidadao Smart 2.3.0" /></div>
                <div className="space-y-1"><FieldLabel>Empresa slug</FieldLabel><input value={form.companySlug} onChange={(event) => setForm((current) => ({ ...current, companySlug: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="testing-company" /></div>
                <div className="space-y-1"><FieldLabel>Empresa nome</FieldLabel><input value={form.companyName} onChange={(event) => setForm((current) => ({ ...current, companyName: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Testing Company" /></div>
                <div className="space-y-1"><FieldLabel>Projeto slug</FieldLabel><input value={form.projectSlug} onChange={(event) => setForm((current) => ({ ...current, projectSlug: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="cid" /></div>
                <div className="space-y-1"><FieldLabel>Responsavel</FieldLabel><input value={form.ownerName} onChange={(event) => setForm((current) => ({ ...current, ownerName: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="QA / Release Manager" /></div>
                <div className="space-y-1"><FieldLabel>Inicio</FieldLabel><input aria-label="Inicio" type="datetime-local" value={form.startAt} onChange={(event) => setForm((current) => ({ ...current, startAt: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
                <div className="space-y-1"><FieldLabel>Fim</FieldLabel><input aria-label="Fim" type="datetime-local" value={form.endAt} onChange={(event) => setForm((current) => ({ ...current, endAt: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
                <div className="space-y-1"><FieldLabel>Perfis que enxergam</FieldLabel><textarea aria-label="Perfis que enxergam" value={form.audienceProfiles} onChange={(event) => setForm((current) => ({ ...current, audienceProfiles: event.target.value }))} className="min-h-24 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="leader_tc, technical_support" /></div>
                <div className="space-y-1"><FieldLabel>Participantes</FieldLabel><textarea aria-label="Participantes" value={form.participantNames} onChange={(event) => setForm((current) => ({ ...current, participantNames: event.target.value }))} className="min-h-24 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Ana, Suporte" /></div>
                <div className="space-y-1 md:col-span-2"><FieldLabel>Descricao</FieldLabel><textarea aria-label="Descricao" value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} className="min-h-20 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="O que precisa acontecer nessa janela." /></div>
                <div className="space-y-1"><FieldLabel>Checklist</FieldLabel><textarea aria-label="Checklist" value={form.checklist} onChange={(event) => setForm((current) => ({ ...current, checklist: event.target.value }))} className="min-h-28 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
                ...
                <div className="space-y-1 md:col-span-2"><FieldLabel>Brain</FieldLabel><textarea aria-label="Brain" value={form.brianRules} onChange={(event) => setForm((current) => ({ ...current, brianRules: event.target.value }))} className="min-h-24 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
              </div>
              ...
              {events.length ? events.map((event) => (
                <article key={event.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <select aria-label={`Alterar status de ${event.title}`} value={event.status} disabled={updatingId === event.id} onChange={(changeEvent) => void handleStatusChange(event.id, changeEvent.target.value)} className="rounded-full border border-(--tc-border,#d7deea) bg-white px-2.5 py-1 text-[11px] font-bold">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select>
                  </div>
