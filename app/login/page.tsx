...
export default function LoginPage() {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < maxDist) {
            ...
            ctx.strokeStyle = `rgba(107,125,183,${alpha * 0.8})`;
            ctx.lineWidth = 1;
            ctx.beginPath();
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-[#041a49] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl bg-white shadow-2xl rounded-xl overflow-hidden grid md:grid-cols-2">
        {/* Faixa/ícone ocupando metade esquerda, sem sombra extra */}
        <div className="hidden md:block min-h-[520px]" aria-label="Testing Company">
          <div
            className="w-full h-full bg-center bg-contain bg-no-repeat"
            style={{ backgroundImage: "url('/images/icone.png')" }}
          />
        </div>
        ...
        <div className="flex flex-col justify-center px-8 py-10 bg-white">
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-full bg-gradient-to-r from-[#e53935] to-[#d81b60] py-2 text-white font-semibold shadow-md hover:opacity-90 disabled:opacity-50 transition"
            >
