...
export async function PATCH(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!canEditManualDefect(role)) {
    return NextResponse.json({ message: "Sem permissÃ£o para atribuir responsÃ¡vel" }, { status: 403 });
  }
  ...
  if (defect.sourceType !== "qase") {
    return NextResponse.json({ message: "Atribuicao local sÃ³ e usada para defeitos integrados" }, { status: 400 });
  }
  ...
  if (!event) {
    return NextResponse.json({ message: "NÃ£o foi possÃ­vel salvar o responsÃ¡vel" }, { status: 500 });
  }
