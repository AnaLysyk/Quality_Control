...
export async function PATCH(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!canMoveTicket(user, item)) {
    return NextResponse.json({ error: "Sem permissÃ£o para alterar status" }, { status: 403 });
  }
  ...
  if (!status || typeof status !== "string") {
    return NextResponse.json({ error: "Campo 'status' obrigatÃ³rio" }, { status: 400 });
  }

  const updated = await updateTicketStatus(id, status, user.id);
  if (!updated) {
    return NextResponse.json({ error: "Chamado nÃ£o encontrado ou status invalido" }, { status: 404 });
  }
