...
export async function PATCH(req: Request, context: { params: Promise<{ commentId: string }> }) {
  ...
  if (!updated) {
    return NextResponse.json({ error: "ComentÃ¡rio invalido" }, { status: 400 });
  }
  ...
  appendTicketEvent({
    ...
    actorUserId: user.id,
    payload: { commentId },
  }).catch((err) => console.error("Falha ao registrar ediÃ§Ã£o:", err));
...
export async function DELETE(req: Request, context: { params: Promise<{ commentId: string }> }) {
  ...
  if (!removed) {
    return NextResponse.json({ error: "Falha ao remover comentÃ¡rio" }, { status: 400 });
  }
  ...
  appendTicketEvent({
    ...
    actorUserId: user.id,
    payload: { commentId },
  }).catch((err) => console.error("Falha ao registrar exclusÃ£o:", err));
