...
export default function NewHomeContent() {
  ...
  async function handleVoiceInput(sendDirectly: boolean) {
    ...
    recognition.onresult = (event) => {
      ...
      if (latestTranscript) showVoiceToast(sendDirectly ? "Entendi. Vou enviar quando você parar de falar." : "Transcrevendo na barra. Revise e envie.");
    };
    recognition.onerror = (event) => {
      const message = voiceErrorMessage(event);
      const canRetry = message.includes("bloqueou") || message.includes("permitir");
      setListening(false);
      setDirectVoiceMode(false);
      recognitionRef.current = null;
      showVoiceToast(message, canRetry ? { actionLabel: "Permitir novamente", actionMode: sendDirectly, duration: 9000 } : { duration: 6500 });
    };
    recognition.onend = () => {
      setListening(false);
      setDirectVoiceMode(false);
      ...
      if (sendDirectly && transcript) { showVoiceToast("Enviando sua fala para o Brian..."); void sendPrompt(transcript); return; }
      if (!sendDirectly && transcript) { showVoiceToast("Texto capturado. Clique em Enviar para o Brian responder."); return; }
      showVoiceToast("Não ouvi nenhuma frase. Tente novamente falando depois que o microfone ficar vermelho.");
    };
  ...
  return (
    <section className="admin-brain-home relative min-h-[calc(100vh-7rem)] w-full overflow-y-auto overflow-x-hidden bg-transparent px-3 pb-44 pt-4 sm:px-6 sm:pb-36 lg:px-10">
      <div className="admin-brain-session-controls fixed right-4 top-20 z-30 flex items-center gap-2 sm:right-8 sm:top-24">
      ...
      <div className="relative z-10 grid min-h-[420px] grid-cols-1 gap-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-6 lg:items-start">
        <aside className="flex justify-center pt-1 lg:justify-start lg:pl-2"><BrainOrb listening={listening} speaking={speaking || isTyping} /></aside>
        <main className="admin-brain-copy mx-auto flex h-[calc(100vh-19rem)] min-h-[260px] w-full max-w-6xl flex-col justify-start gap-4 overflow-hidden pt-2 text-left sm:h-[calc(100vh-17rem)] lg:h-[calc(100vh-15rem)] lg:pt-12 lg:pr-12">
          <div className="admin-brain-chat-list space-y-3 overflow-y-auto overflow-x-hidden pr-1 sm:pr-2">
            {messages.map((message, index) => { const isLatestAssistant = message.role === "assistant" && index === latestAssistantIndex; const displayText = isLatestAssistant ? typedAssistantText : message.text; return <div key={message.id} className={`flex min-w-0 ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`admin-brain-message max-w-[92%] overflow-hidden rounded-3xl px-4 py-3 sm:max-w-[82%] sm:px-5 sm:py-4 ${message.role === "user" ? "admin-brain-message-user text-right" : "admin-brain-message-assistant"}`}><p className="min-w-0 whitespace-pre-wrap break-words text-sm font-semibold leading-relaxed sm:text-xl">{displayText}{isLatestAssistant ? <motion.span className="ml-1 inline-block h-5 w-[2px] translate-y-1 bg-red-500" animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 0.9 }} /> : null}</p></div></div>; })}
            <div ref={messagesEndRef} />
          </div>
        </main>
      </div>
      <div className="admin-brain-actions fixed bottom-[7.1rem] left-3 right-3 z-30 flex max-h-24 flex-wrap items-stretch gap-2 overflow-y-auto overflow-x-hidden sm:left-6 sm:right-6 lg:left-[13.25rem] lg:right-[6.25rem]">
