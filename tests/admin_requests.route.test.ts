...
describe("/api/admin/requests GET", () => {
  ...
  it("retorna 403 se não for admin", async () => {
    ...
    const res = await GET(new (require("next/server").NextRequest)("http://localhost/api/admin/requests"));
  ...
  it("retorna lista se admin", async () => {
    ...
    const res = await GET(new (require("next/server").NextRequest)("http://localhost/api/admin/requests?status=PENDING"));
