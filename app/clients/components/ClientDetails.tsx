...
export function ClientDetails({ client, isGlobalAdmin, onOpenCreateUser, onEditClient }: Props) {
  ...
  const infoFields = [
    { label: "CNPJ", value: client.taxId ?? "Ṇo informado" },
    { label: "CEP", value: client.zip ?? "Ṇo informado" },
    { label: "Endere̤o", value: client.address ?? "Ṇo informado" },
    { label: "Website", value: client.website ?? "Ṇo informado", isLink: true },
    { label: "Telefone", value: client.phone ?? "Ṇo informado" },
    { label: "Documenta̤̣o", value: client.docsLink ?? "Ṇo informado", isLink: true },
    { label: "LinkedIn", value: client.linkedin ?? "Ṇo informado", isLink: true },
    { label: "Descri̤̣o", value: client.description ?? "Sem descri̤̣o", full: true },
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-900">Informa̵̤es gerais</h3>
          {isGlobalAdmin && (
            <button
...
function Field({ label, value, isLink, full }: { label: string; value: string; isLink?: boolean; full?: boolean }) {
  const display = value ?? "Ṇo informado";
  const trimmed = display.trim();
  const isPlaceholder =
    display === "Ṇo informado" || display === "Sem notas" || display === "Sem descri̤̣o";
