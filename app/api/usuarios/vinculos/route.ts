...
function canManage(user: Awaited<ReturnType<typeof authenticateRequest>>) {
  ...
  const role = String(user.permissionRole ?? user.role ?? user.companyRole ?? "").trim().toLowerCase();
  return Boolean(
    user.isGlobalAdmin ||
      role === "leader_tc" ||
      role === "empresa" ||
...
export async function POST(req: Request) {
  ...
  try {
    const result = await db.$transaction(async (tx) => {
      ...
      const companyMembership = await tx.membership.findUnique({
        where: { userId_companyId: { userId: target.id, companyId: company.id } },
      });
      const companyLink = await tx.userCompanyLink.findUnique({
        where: { userId_companyId: { userId: target.id, companyId: company.id } },
      });
      if (!companyMembership && !companyLink) {
        throw new Error("A pessoa precisa estar vinculada à empresa antes de entrar no projeto");
      }
      ...
      const duplicate = await tx.projectTeamAssignment.findFirst({
        where: { userId: target.id, projectId: project.id, role: body.role, status: "active" },
      });
    ...
    writeAuditLog({
      ...
      metadata: { companyId: result.company.id, projectId: result.project.id, role: body.role },
    });
  ...
  try {
    const result = await db.$transaction(async (tx) => {
      ...
      if (assignment.role === "leader_tc") {
        throw new Error("O Líder TC não pode ser removido diretamente. Realize a troca de liderança para manter o projeto com um líder ativo");
      }
    ...
    await createNotificationsForUsers([result.user.id, result.leaderUserId ?? ""], {
      type: "RELATIONSHIP_REMOVED",
      title: "Vínculo de projeto removido",
      description: `O vínculo de ${result.user.name} com ${result.project.name} foi removido. Motivo: ${body.reason!.trim()}`,
      companySlug: result.company.slug,
      projectSlug: result.project.slug,
      actorId: user.id,
      actorName: user.email,
