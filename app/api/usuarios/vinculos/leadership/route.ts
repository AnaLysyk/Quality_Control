...
type TargetUser = { id: string; name: string; full_name?: string | null; email: string };
...
async function assertCompanyScope(user: AuthUser, companyId: string) {
  ...
  if (scopedIds && !scopedIds.has(companyId)) throw new Error("Empresa fora do seu contexto autorizado");
}
...
function isStrictLeader(target: {
  ...
  return Boolean(
    target.role === Role.leader_tc ||
      ...
      target.memberships?.some((membership) => membership.role === Role.leader_tc),
  );
}
...
export async function GET(req: Request) {
  ...
  const currentLeader = assignments.find((item) => item.role === "leader_tc") ?? null;
  return NextResponse.json({
    project,
    leader: currentLeader,
    qaUsers: assignments.filter((item) => item.role === "qa_tc"),
    leaderCandidates: leaderCandidates.filter((candidate) => candidate.id !== currentLeader?.user.id),
    qaCandidates,
    permissions: {
...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => null)) as LeadershipBody | null;
  ...
  const action: LeadershipAction = body.action;
  ...
  try {
    await assertCompanyScope(user, project.companyId);
    if (body.companyId && body.companyId !== project.companyId) throw new Error("O projeto não pertence à empresa informada");

    const result: OperationResult = await db.$transaction(async (tx): Promise<OperationResult> => {
      if (action === "assign_leader") {
        ...
        if (!isStrictLeader(target)) throw new Error("A pessoa selecionada não possui perfil de Líder TC");
        ...
        return { action: "assign_leader", project, target, assignment, notifyIds: [target.id] };
      }

      if (action === "transfer_leader") {
        if (!body.newLeaderId) throw new Error("Selecione o novo Líder TC");
        const reason = body.reason?.trim();
        if (!reason) throw new Error("Informe a justificativa da transferência");
        ...
        await tx.projectTeamAssignment.update({
          where: { id: current.id },
          data: { status: "removed", removedBy: user.id, removedAt: new Date(), removalReason: reason },
        });
        const assignment = await tx.projectTeamAssignment.create({
          data: { userId: target.id, companyId: project.companyId, projectId: project.id, role: "leader_tc", createdBy: user.id },
          select: { id: true },
      ...
      if (action === "add_qa") {
        ...
        return { action: "add_qa", project, target, assignment, notifyIds: [target.id, leader.userId] };
      }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Falha ao atualizar liderança ou equipe" },
      { status: 400 },
    );
  }
}
