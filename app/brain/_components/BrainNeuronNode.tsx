...
function statusLabel(node: BrainNode, connectedCount: number) {
  ...
  const status = typeof node.status === "string" && node.status ? node.status : "ok";
...
function theme(node: BrainNode, selected: boolean, orphan: boolean, darkMode: boolean) {
  if (darkMode) {
    ...
    if (["pending", "warning"].includes(String(node.status ?? ""))) return "border-yellow-300/80 bg-yellow-950/94 text-yellow-50 shadow-[0_0_36px_rgba(250,204,21,.28),inset_0_0_18px_rgba(250,204,21,.12)]";
  ...
  if (["pending", "warning"].includes(String(node.status ?? ""))) return "border-yellow-300 bg-white text-yellow-950 shadow-[0_0_24px_rgba(250,204,21,.18),inset_0_0_16px_rgba(250,204,21,.08)]";
...
function nodeKicker(node: BrainNode) {
  ...
  return node.module || node.type || "Contexto";
}
...
function nodeInstruction(node: BrainNode) {
  ...
  return labelize(node.type || node.module || "contexto");
}
