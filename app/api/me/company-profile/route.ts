...
export async function PATCH(req: NextRequest) {
  ...
  if (!canManageInstitutionalCompanyAccess(access)) {
    return NextResponse.json({ error: "Sem permissÃ£o para editar o perfil institucional da empresa" }, { status: 403 });
  }
  ...
  if (!nextName) {
    return NextResponse.json({ error: "Nome da empresa obrigatÃ³rio" }, { status: 400 });
  }
  ...
  if (duplicateByName) {
    return NextResponse.json({ error: "Empresa jÃ¡ cadastrada com esse nome" }, { status: 409 });
  }
  ...
  const duplicateByTaxId =
    nextTaxId.length > 0
      ...
      : null;
  if (duplicateByTaxId) {
    return NextResponse.json({ error: "CNPJ jÃ¡ cadastrado para outra empresa" }, { status: 409 });
  }
  const qaseTouched =
    ...
    if (hasNextQaseToken || hasNextQaseProjects) {
      if (!hasNextQaseToken || !hasNextQaseProjects) {
        return NextResponse.json(
          { error: "A integraÃ§Ã£o da Qase sÃ³ pode ser salva quando estiver ativa e vÃ¡lida, com token e projetos confirmados." },
          { status: 400 },
        );
      }
      try {
        ...
        const message =
          error instanceof QaseError && (error.status === 401 || error.status === 403)
            ...
            : "NÃ£o foi possÃ­vel validar a integraÃ§Ã£o da Qase. Busque os projetos novamente antes de salvar.";
  ...
  if (!clearAllIntegrations && jiraTouched) {
    ...
    if (hasAnyJiraConfig) {
      if (!normalizedNextJiraBaseUrl?.trim() || !normalizedNextJiraEmail?.trim() || !normalizedNextJiraApiToken?.trim()) {
        return NextResponse.json(
          { error: "A integraÃ§Ã£o do Jira sÃ³ pode ser salva quando estiver ativa e vÃ¡lida, com URL, e-mail tÃ©cnico e API token." },
          { status: 400 },
        );
      }
