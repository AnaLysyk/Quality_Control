    ...
    <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-4 shadow-sm sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-3 text-xl font-extrabold text-(--tc-text,#0b1a3c)">Solicitações públicas e fluxo de e-mails</h2>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-4 py-3">
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{label}</p>
            <p className="mt-1 text-2xl font-black text-(--tc-text,#0b1a3c)">{isLoading ? "..." : value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-3">
          ...
          <div className="mt-3 space-y-2">
            {(data?.routes ?? []).map((route) => (
              <div key={route.id} className="rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-(--tc-text,#0b1a3c)">{route.label}</p>
                    <p className="mt-1 truncate font-mono text-xs text-(--tc-text-muted,#6b7280)">{route.method} {route.path}</p>
                  </div>
            ...
            {!data?.routes?.length && !isLoading ? <p className="text-sm text-(--tc-text-muted,#6b7280)">Nenhuma rota retornada.</p> : null}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <FiMail className="h-5 w-5 text-(--tc-accent,#ef0001)" />
              <h3 className="text-sm font-extrabold text-(--tc-text,#0b1a3c)">Validação por solicitação</h3>
            </div>
            <span className="inline-flex items-center gap-1 rounded-full border border-(--tc-border,#d7deea) bg-white px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-(--tc-text-muted,#6b7280)">
          ...
          <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(220px,0.45fr)_minmax(0,0.55fr)]">
            <div className="max-h-96 space-y-2 overflow-auto pr-1">
              {(data?.requests ?? []).map((request) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border px-3 py-2 text-left ${selected ? "border-(--tc-accent,#ef0001) bg-white" : "border-(--tc-border,#d7deea) bg-white/70"}`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-sm font-bold text-(--tc-text,#0b1a3c)">{request.requesterName ?? request.requesterEmail}</p>
                        <p className="mt-1 text-xs text-(--tc-text-muted,#6b7280)">{statusLabel(request.status)} • {request.emails.length} e-mail(s)</p>
                      </div>
              ...
              {!data?.requests?.length && !isLoading ? <p className="text-sm text-(--tc-text-muted,#6b7280)">Nenhuma solicitação encontrada.</p> : null}
            </div>

            <div className="rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-3">
              {selectedRequest ? (
                <div>
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-base font-extrabold text-(--tc-text,#0b1a3c)">{selectedRequest.requesterEmail}</p>
                      <p className="mt-1 text-xs text-(--tc-text-muted,#6b7280)">Perfil: {selectedRequest.requestedRole ?? "--"} • Criado: {formatDate(selectedRequest.createdAt)}</p>
                    </div>
                    <span className="rounded-full bg-(--tc-surface-2,#f8fafc) px-2.5 py-1 text-xs font-bold text-(--tc-text-muted,#6b7280)">{statusLabel(selectedRequest.status)}</span>
                  </div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {[
                      ...
                      <div key={String(label)} className="flex items-center gap-2 rounded-lg border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-2.5 py-2 text-xs font-bold text-(--tc-text,#0b1a3c)">
                  ...
                  <div className="mt-4 space-y-2">
                    <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">E-mails enviados</p>
                    {selectedRequest.emails.map((email) => (
                      <div key={email.id} className="rounded-xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-3 py-2">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="truncate text-sm font-bold text-(--tc-text,#0b1a3c)">{email.subject}</p>
                            <p className="mt-1 text-xs text-(--tc-text-muted,#6b7280)">{kindLabel(email.kind)} • {formatDate(email.createdAt)} • {email.source}</p>
                          </div>
                          <button
                            type="button"
                            onClick={() => openEmailPreview(email)}
                            className="inline-flex h-8 shrink-0 items-center gap-1 rounded-lg border border-(--tc-border,#d7deea) bg-white px-2 text-xs font-bold text-(--tc-text,#0b1a3c)"
                          >
                    ...
                    {!selectedRequest.emails.length ? <p className="rounded-xl border border-dashed border-(--tc-border,#d7deea) px-3 py-3 text-sm text-(--tc-text-muted,#6b7280)">Nenhum e-mail capturado para esta solicitação.</p> : null}
                  </div>
                </div>
              ) : (
                <p className="text-sm text-(--tc-text-muted,#6b7280)">Selecione uma solicitação para ver logs e e-mails.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {data?.notes?.length ? (
        <div className="mt-4 rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) px-3 py-2 text-xs leading-5 text-(--tc-text-muted,#6b7280)">
