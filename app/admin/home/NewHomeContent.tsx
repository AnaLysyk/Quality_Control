...
import { FiMic, FiMessageCircle, FiSend, FiTrash2, FiVolume2, FiVolumeX } from "react-icons/fi";
...
export default function NewHomeContent() {
  ...
  const [messages, setMessages] = useState<BrainMessage[]>(() => [{ id: "assistant-initial", role: "assistant", text: initialAssistantText }]);
  ...
  function speakBrain(text: string) {
    ...
    utterance.lang = "pt-BR"; utterance.rate = 0.92; utterance.pitch = 1.02; utterance.volume = volume;
  ...
  function sendPrompt(prompt: string) {
    ...
    const now = Date.now();
    setMessages((current) => [...current, { id: `user-${now}`, role: "user", text: cleanPrompt }, { id: `assistant-${now}`, role: "assistant", text: buildAssistantReply(cleanPrompt, roleLabel) }]);
  ...
  function clearChat() {
    ...
    setSpeaking(false); setCommand(""); setMessages([{ id: `assistant-reset-${Date.now()}`, role: "assistant", text: initialAssistantText }]);
  }
  function handleVoiceInput(sendDirectly: boolean) {
    const SpeechRecognition = resolveSpeechRecognition();
    if (!SpeechRecognition) {
      setSpeechSupported(false);
      setMessages((current) => [...current, { id: `assistant-speech-${Date.now()}`, role: "assistant", text: "Seu navegador não liberou reconhecimento de voz agora. Digite a mensagem ou use o chat lateral." }]);
      return;
    }
    const recognition = new SpeechRecognition();
    let finalTranscript = "";
    recognition.lang = "pt-BR"; recognition.interimResults = true; recognition.continuous = false;
    setListening(true); setDirectVoiceMode(sendDirectly); if (sendDirectly) setVoiceEnabled(true);
    recognition.onresult = (event) => {
      let transcript = "";
      for (let index = event.resultIndex; index < event.results.length; index += 1) { transcript += event.results[index][0].transcript; if (event.results[index].isFinal) finalTranscript = transcript.trim(); }
      setCommand(transcript.trim());
    };
    recognition.onerror = () => { setListening(false); setDirectVoiceMode(false); setMessages((current) => [...current, { id: `assistant-error-${Date.now()}`, role: "assistant", text: "Não consegui captar o áudio. Tente novamente ou digite sua mensagem." }]); };
    recognition.onend = () => { setListening(false); setDirectVoiceMode(false); if (sendDirectly && finalTranscript) sendPrompt(finalTranscript); };
    recognition.start();
  }
  ...
  return (
    <section className="admin-brain-home relative min-h-[calc(100vh-7rem)] w-full overflow-hidden bg-transparent px-4 pb-28 pt-5 sm:px-8 lg:px-10">
      ...
      <div className="admin-brain-actions fixed bottom-[5.6rem] left-[13.25rem] right-[6.25rem] z-30 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-7 max-md:left-4 max-md:right-[5.75rem]">
        ...
        <button type="button" onClick={clearChat} className="admin-brain-action rounded-2xl border border-white/10 bg-white/[0.025] px-3 py-2 text-left transition hover:border-red-400/50 hover:bg-red-500/10"><span className="flex items-center gap-1 text-xs font-bold text-white"><FiTrash2 className="h-3.5 w-3.5" /> Limpar</span><span className="mt-0.5 block truncate text-[10px] text-slate-400">reiniciar conversa</span></button>
      </div>
