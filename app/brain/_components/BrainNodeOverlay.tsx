...
import { FiActivity, FiExternalLink, FiMove, FiRefreshCw, FiSave, FiX, FiZap } from "react-icons/fi";
...
export function BrainNodeOverlay({ node, nodes, edges, onClose, onResetFocus, onOpenRelatedModule, debugMode = false }: BrainNodeOverlayProps) {
    ...
    try {
      const response = await fetch("/api/brain/memory", {
        ...
        body: JSON.stringify({
          ...
          sourceType: "MANUAL",
          importance: 3,
          metadata: { nodeLabel: node.label, nodeModule: node.module, nodeType: node.type, origin: "brain-node-overlay" },
        }),
      });
      if (!response.ok) throw new Error("Não foi possível salvar a memória.");
      setMemoryTitle("");
      setMemorySummary("");
    ...
    <div className="pointer-events-none fixed inset-0 z-[9999] text-white">
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <section className="qc-brain-rag-context-card mt-3 rounded-2xl border border-cyan-100/14 bg-cyan-100/[0.055] p-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/55">Contexto RAG recuperado</p><h3 className="mt-1 text-sm font-black text-white">Memória viva deste neurônio</h3></div>
              <span className="rounded-full border border-emerald-200/20 bg-emerald-200/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-emerald-100">{ragLoading ? "buscando" : ragContext?.source ?? "contexto"}</span>
            </div>
            {ragLoading ? <p className="mt-3 text-xs font-bold text-slate-200/78">Consultando cérebro, permissões, eventos, notas e documentos vinculados...</p> : ragContext ? (
              <div className="mt-3 grid gap-3">
                ...
                {ragContext.memory?.length ? <div className="rounded-xl border border-white/10 bg-black/18 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-100/45">Memórias encontradas</p><div className="mt-2 grid gap-2">{ragContext.memory.slice(0, 4).map((item) => <div key={item.id} className="rounded-lg border border-white/10 bg-white/[0.035] px-3 py-2"><p className="text-xs font-black text-slate-50">{item.title ?? item.kind ?? "Memória"}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{item.kind ?? "contexto"}{item.status ? ` · ${item.status}` : ""}</p></div>)}</div></div> : <p className="rounded-xl border border-white/10 bg-black/18 p-3 text-xs font-bold text-slate-200/72">Nenhuma memória adicional retornou ainda. O Brain deve completar este contexto automaticamente com logs, histórico, documentos, eventos e relações do sistema.</p>}
              </div>
            ) : <p className="mt-3 text-xs font-bold text-slate-200/78">Contexto RAG ainda não respondeu para este nó.</p>}
          </section>

          <section className="mt-3 rounded-2xl border border-cyan-100/14 bg-slate-950/40 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/45">Ensinar contexto para este nó</p>
            <input value={memoryTitle} onChange={(event) => setMemoryTitle(event.target.value)} placeholder="Título da memória" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-slate-500 focus:border-cyan-200/40" />
            <textarea value={memorySummary} onChange={(event) => setMemorySummary(event.target.value)} placeholder="O que o Brain precisa lembrar sobre este nó?" rows={3} className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-slate-500 focus:border-cyan-200/40" />
            <div className="mt-2 flex items-center justify-between gap-2"><p className="text-[10px] font-bold text-slate-400">{memoryFeedback ?? "Salva contexto manual no Brain."}</p><button type="button" onClick={saveNodeMemory} disabled={savingMemory} className="inline-flex items-center gap-2 rounded-xl border border-cyan-100/25 bg-cyan-100/10 px-3 py-2 text-xs font-black text-cyan-50 hover:border-cyan-100/60 disabled:opacity-60"><FiSave className="h-3.5 w-3.5" />{savingMemory ? "Salvando" : "Salvar"}</button></div>
          </section>

          {metadata.length ? <section className="mt-3 rounded-2xl border border-white/10 bg-slate-950/40 p-3"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/45">Dados conhecidos</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{metadata.map((item) => <div key={item.key} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">{formatKey(item.key)}</p><p className="mt-1 break-words text-xs font-bold text-slate-100">{item.value}</p></div>)}</div></section> : null}
          ...
          <section className="mt-3 rounded-2xl border border-white/10 bg-slate-950/40 p-3"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/45">Ações disponíveis</p><div className="mt-2 grid gap-2">{actions.map((action) => <button key={action} type="button" onClick={() => onOpenRelatedModule(node.module)} className="flex items-center gap-2 rounded-xl border border-cyan-100/12 bg-cyan-100/[0.06] px-3 py-2 text-left text-xs font-bold text-cyan-50 hover:border-cyan-100/35"><FiZap className="h-3.5 w-3.5 shrink-0" />{action}</button>)}</div></section>
        ...
        <footer className="flex items-center justify-between gap-2 border-t border-white/10 bg-slate-950/72 p-3">
          <button type="button" onClick={onResetFocus} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-black text-white/70 hover:border-cyan-100/35 hover:text-white"><FiRefreshCw className="h-3.5 w-3.5" />Limpar foco</button>
          <button type="button" onClick={askBrainChatAboutNode} className="qc-brain-ask-chat-button inline-flex items-center gap-2 rounded-xl border border-cyan-100/25 bg-cyan-100/10 px-3 py-2 text-xs font-black text-cyan-50 hover:border-cyan-100/60"><FiZap className="h-3.5 w-3.5" />Perguntar ao Brain Chat</button>
          <button type="button" onClick={() => onOpenRelatedModule(node.module)} className="inline-flex items-center gap-2 rounded-xl border border-cyan-100/25 bg-cyan-100/10 px-3 py-2 text-xs font-black text-cyan-50 hover:border-cyan-100/60"><FiActivity className="h-3.5 w-3.5" />Ver área<FiExternalLink className="h-3.5 w-3.5" /></button>
        </footer>
      </article>
    </div>,
    document.body,
