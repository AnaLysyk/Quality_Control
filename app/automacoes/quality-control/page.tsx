    ...
    <main className="min-h-screen bg-(--page-bg,#f5f7fb) px-4 py-6 text-(--page-text,#0b1a3c) sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-5">
        <section className="rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-black tracking-tight text-[var(--tc-text,#0b1a3c)]">{internalQualityControlCompany.name}</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">{internalQualityControlCompany.summary}</p>
            </div>
            <div className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-primary,#011848)] px-4 py-3 text-white">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <article className="rounded-[24px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-4 shadow-sm lg:col-span-2">
            <div className="flex items-center gap-2">
              <FiLayers className="h-5 w-5 text-[var(--tc-accent,#ef0001)]" />
              <h2 className="text-lg font-extrabold text-[var(--tc-text,#0b1a3c)]">Projetos internos do produto</h2>
            </div>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              {internalQualityControlCompany.projects.map((project) => (
                <div key={project.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-base font-extrabold text-[var(--tc-text,#0b1a3c)]">{project.name}</p>
                      <p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-[var(--tc-text-muted,#6b7280)]">{project.area}</p>
                    </div>
                    <span className="rounded-full border border-[var(--tc-border,#d7deea)] bg-white px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--tc-text-muted,#6b7280)]">{project.priority}</span>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">{project.goal}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {project.suites.map((suite) => (
                      <span key={suite} className="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-[var(--tc-text,#0b1a3c)]">{suite}</span>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <aside className="space-y-4">
            <article className="rounded-[24px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-4 shadow-sm">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-extrabold text-[var(--tc-text,#0b1a3c)]">Regras Playwright</h2>
              </div>
              <ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">
                {playwrightQualityControlRules.map((rule) => (
                  <li key={rule} className="rounded-xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-3 py-2">{rule}</li>
                ))}
              </ul>
            </article>
            ...
            <article className="rounded-[24px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#fff)] p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-2">
                {flow.map((item, index) => (
                  <div key={item} className="flex items-center gap-2 rounded-xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-3 py-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">
                    <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[var(--tc-primary,#011848)] text-xs font-black text-white">{index + 1}</span>
                    {item}
                  </div>
                ))}
              </div>
