...
const EDITOR_TOOLS: EditorTool[] = [
  { id: "h1", label: "H1", title: "TÃ­tulo", group: "structure" },
  ...
  { id: "code", label: "</>", title: "Bloco de cÃ³digo", group: "style" },
  { id: "json", label: "{ }", title: "JSON formatado", group: "style" },
  { id: "link", label: "Link", title: "Link clicÃ¡vel", group: "style" },
];
...
const NOTE_SNIPPETS: NoteSnippet[] = [
  {
    id: "decision",
    label: "DecisÃ£o",
    title: "Inserir bloco de decisÃ£o",
    text: "## DecisÃ£o\n- Contexto:\n- DefiniÃ§Ã£o:\n- Impacto:\n",
  },
  {
    id: "nextSteps",
    label: "PrÃ³ximos passos",
    title: "Inserir bloco de prÃ³ximos passos",
  ...
  },
  {
    id: "linkBlock",
    label: "ReferÃªncias",
    title: "Inserir bloco de referÃªncias",
    text: "## ReferÃªncias\n- [Link principal](https://exemplo.com)\n",
  },
  {
    id: "codeBlock",
    label: "CÃ³digo",
    title: "Inserir bloco de cÃ³digo",
...
function normalizeNoteItem(note: Partial<NoteItem> | null | undefined): NoteItem {
  return {
    ...
    title: typeof note?.title === "string" && note.title.trim() ? note.title.trim().slice(0, 120) : "Sem tÃ­tulo",
...
function getEditorToolFeedback(toolId: EditorTool["id"]) {
  switch (toolId) {
    case "h1":
      return "TÃ­tulo principal aplicado na linha atual.";
    case "h2":
      return "Subtitulo aplicado na linha atual.";
    case "h3":
      return "SeÃ§Ã£o inserida para organizar o conteÃºdo.";
    case "list":
    ...
    case "bold":
      return "Negrito aplicado na seleÃ§Ã£o.";
    case "italic":
      return "ItÃ¡lico aplicado na seleÃ§Ã£o.";
    case "highlight":
      return "Destaque aplicado na seleÃ§Ã£o.";
    case "code":
      return "Bloco de cÃ³digo inserido.";
    case "json":
...
function getNotePreview(content: string, maxLength = 112) {
  ...
  if (!plain) return "Abra a nota para escrever checklist, links, cÃ³digo ou observaÃ§Ãµes mais completas.";
...
function formatNoteForClipboard(note: Pick<NoteItem, "title" | "content" | "status" | "priority" | "updatedAt" | "createdAt">) {
  return [
    ...
    "",
    `TÃ­tulo: ${note.title || "Sem tÃ­tulo"}`,
    ...
    "",
    "DescriÃ§Ã£o:",
    getPlainNoteContent(note.content) || "Sem descriÃ§Ã£o.",
  ].join("\n");
}
...
function renderRichContent(content: string) {
  if (!content.trim()) {
    return <p className="notes-rich-empty">Sem conteÃºdo ainda. Use tÃ­tulos, checklist, links ou cÃ³digo para estruturar melhor a nota.</p>;
  }
            ...
            <div className="notes-editor-modern-workspace" data-panel={activePanel}>
              <div className={`notes-editor-modern-write ${activePanel === "preview" ? "hidden" : ""}`}>
                ...
                <div className="notes-editor-meta notes-editor-meta-modern">
                  <span className="notes-card-inline-label">ConteÃºdo</span>
              ...
              {activePanel === "preview" ? (
                <div className="notes-editor-modern-preview">
                  <div className="notes-editor-live-header">
                    <span className="notes-card-inline-label">VisualizaÃ§Ã£o</span>
...
export default function NotesButton({ defaultOpen = false }: NotesButtonProps) {
  ...
  const applyEditorTool = useCallback(
    (toolId: EditorTool["id"]) => {
      ...
      if (!applied && toolId === "json") {
        setError("Selecione um JSON vÃ¡lido para formatar ou use o bloco JSON como modelo.");
        setEditorLastAction("NÃ£o foi possÃ­vel formatar o JSON selecionado.");
  ...
  async function copyNote(note: Pick<NoteItem, "title" | "content" | "status" | "priority" | "updatedAt" | "createdAt">) {
    ...
    try {
      ...
      setError(copyError instanceof Error ? copyError.message : "NÃ£o foi possÃ­vel copiar a nota");
    }
  }
  ...
  async function exportNotePdf(note: Pick<NoteItem, "title" | "content" | "status" | "priority" | "updatedAt" | "createdAt">) {
    ...
    try {
      ...
      const drawFooter = (pageNumber: number) => {
        ...
        doc.text(`PÃ¡gina ${pageNumber}`, pageWidth - marginX, footerY, { align: "right" });
      };
      ...
      const titleLines = doc.splitTextToSize(note.title || "Sem tÃ­tulo", contentWidth);
      ...
      doc.setTextColor(11, 26, 60);
      doc.text("DescriÃ§Ã£o", marginX, cursorY);
      cursorY += 18;

      const contentLines = doc.splitTextToSize(getPlainNoteContent(note.content) || "Sem descriÃ§Ã£o.", contentWidth - 32);
      const lineHeight = 16;
      let lineIndex = 0;
      ...
      while (lineIndex < contentLines.length) {
        if (cursorY > pageHeight - 120) {
          ...
          doc.setFontSize(11.5);
          doc.setTextColor(11, 26, 60);
          doc.text("DescriÃ§Ã£o (continua)", marginX, cursorY);
          cursorY += 18;
        }
      ...
      setError(exportError instanceof Error ? exportError.message : "NÃ£o foi possÃ­vel exportar a nota em PDF");
    }
  }
  ...
  const activeToast = error
    ? { tone: "error" as const, title: "AtenÃ§Ã£o", text: error }
    : message
  ...
  return (
    <div className="relative h-11 w-11 shrink-0 overflow-visible" ref={boxRef}>
      ...
      >
        <FiEdit3 size={18} />
        {notes.length > 0 ? (
          <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[var(--tc-accent,#ef0001)] text-[9px] font-black text-white">
                    ...
                    <div className="notes-widget-toolbar">
                      <div className="notes-toolbar-primary">
                        <div className="notes-search-shell">
                          ...
                          <input
                            type="text"
                            placeholder="Buscar tÃ­tulo, descriÃ§Ã£o, conteÃºdo, status, prioridade ou cor"
                            value={search}
                              ...
                              >
                                ...
                                >
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="notes-card-title">{liveNote.title || "Sem tÃ­tulo"}</p>
