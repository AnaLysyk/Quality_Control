...
const {
  ...
  ehFormatoWsq,
} = require('../codigo-fonte/processar-imagem-digital');
const {
  FingerprintBase64ExceededError,
  InvalidFingerprintImageError,
} = require('../codigo-fonte/erros');
const { MAX_BASE64_LENGTH } = require('../codigo-fonte/constantes');
...
describe('Utilitários auxiliares', () => {
  test('converterBufferParaBase64 converte buffer corretamente', () => {
    const buf = Buffer.from('hello world');
    const b64 = converterBufferParaBase64(buf);
    expect(b64).toBe(buf.toString('base64'));
    expect(typeof b64).toBe('string');
  });

  test('estimarTamanhoBase64 calcula corretamente', () => {
    // 3 bytes → 4 chars
    expect(estimarTamanhoBase64(3)).toBe(4);
    // 6 bytes → 8 chars
    expect(estimarTamanhoBase64(6)).toBe(8);
    // 1 byte → 4 chars (com padding)
    expect(estimarTamanhoBase64(1)).toBe(4);
    // 0 bytes → 0 chars
    expect(estimarTamanhoBase64(0)).toBe(0);
  });

  test('calcularMaximoBytesParaBase64 calcula inverso corretamente', () => {
    const maxChars = 500_000;
    const maxBytes = calcularMaximoBytesParaBase64(maxChars);
    // O Base64 de maxBytes deve caber em maxChars
    expect(estimarTamanhoBase64(maxBytes)).toBeLessThanOrEqual(maxChars);
    // E maxBytes+1 ultrapassaria
    expect(estimarTamanhoBase64(maxBytes + 3)).toBeGreaterThan(maxChars);
  });

  test('ehFormatoWsq detecta magic bytes WSQ corretamente', () => {
    expect(ehFormatoWsq(Buffer.from([0xFF, 0xA0, 0x00]))).toBe(true);
    expect(ehFormatoWsq(Buffer.from([0xFF, 0xA1, 0x00]))).toBe(false);
    expect(ehFormatoWsq(Buffer.from([0x89, 0x50]))).toBe(false); // PNG magic
    expect(ehFormatoWsq(null)).toBe(false);
    expect(ehFormatoWsq(Buffer.alloc(0))).toBe(false);
    expect(ehFormatoWsq(Buffer.alloc(1))).toBe(false);
  });
});

describe('ajustarDigitalAoLimiteBase64', () => {
  ...
  test('retorna imagem sem redução quando já está dentro do limite', async () => {
    ...
    const result = await ajustarDigitalAoLimiteBase64(smallPng, {
  ...
  test('reduz imagem quando Base64 excede o limite', async () => {
    ...
    const result = await ajustarDigitalAoLimiteBase64(largePng, {
  ...
  test('lança FingerprintBase64ExceededError quando não consegue reduzir', async () => {
    ...
    await expect(
      ajustarDigitalAoLimiteBase64(noisyPng, {
    ...
    try {
      await ajustarDigitalAoLimiteBase64(noisyPng, {
        maxBase64Length: impossibleLimit,
        maxAttempts: 3,
  ...
  test('valida pelo tamanho da string Base64, não pelos bytes do arquivo', async () => {
    ...
    if (base64Length > limitBetweenBytesAndBase64) {
      ...
      const result = await ajustarDigitalAoLimiteBase64(png, {
  ...
  test('WSQ dentro do limite é retornado sem alteração', async () => {
    ...
    const result = await ajustarDigitalAoLimiteBase64(wsqBuffer, {
  ...
  test('WSQ acima do limite lança FingerprintBase64ExceededError', async () => {
    ...
    await expect(
      ajustarDigitalAoLimiteBase64(largeWsq, {
  ...
  test('lança InvalidFingerprintImageError para buffer vazio', async () => {
    await expect(
      ajustarDigitalAoLimiteBase64(Buffer.alloc(0))
    ).rejects.toThrow(InvalidFingerprintImageError);
  });

  test('lança InvalidFingerprintImageError para input não-Buffer', async () => {
    await expect(
      ajustarDigitalAoLimiteBase64('not a buffer')
    ).rejects.toThrow(InvalidFingerprintImageError);

    await expect(
      ajustarDigitalAoLimiteBase64(null)
    ).rejects.toThrow(InvalidFingerprintImageError);
  });
  ...
  test('regressão: PNG 640x600 com ruído cabe em 500.000 chars Base64', async () => {
    ...
    const result = await ajustarDigitalAoLimiteBase64(realisticPng, {
  ...
  test('JPEG de entrada é processado corretamente', async () => {
    ...
    const result = await ajustarDigitalAoLimiteBase64(jpegBuffer, {
...
// ============================================================
// Testes do modo ABOVE (aumentarImagemAteTamanhoAlvo)
// ============================================================
describe('aumentarImagemAteTamanhoAlvo (modo ABOVE)', () => {
  // --------------------------------------------------------
  ...
  test('infla imagem até Base64 atingir o alvo', async () => {
    ...
    const originalBase64 = converterBufferParaBase64(smallPng);
    const alvo = originalBase64.length * 3; // alvo 3x o tamanho original

    const result = await aumentarImagemAteTamanhoAlvo(smallPng, 320, 300, alvo, 10, 'test-inflate');
  ...
  test('retorna sem upscale se imagem já excede alvo', async () => {
    ...
    const result = await aumentarImagemAteTamanhoAlvo(largePng, 640, 600, alvo, 10, 'test-inflate-already');
  ...
  test('infla imagem para ultrapassar 500.000 chars Base64', async () => {
    ...
    const result = await aumentarImagemAteTamanhoAlvo(png, 320, 300, 500_001, 10, 'test-inflate-wallet');
...
describe('Bug Wallet — dados reais do ticket (processo 43229358)', () => {
  ...
  // --------------------------------------------------------
  // CASO 12: reduzirImagemPng deve conseguir reduzir dedo 8 abaixo de 500K
  // --------------------------------------------------------
  (hasRealData ? test : test.skip)(
    'ajustarDigitalAoLimiteBase64 reduz dedo 8 real para ≤ 500K chars',
    async () => {
      const b64Original = fs.readFileSync(DEDO8_FILE, 'utf-8').trim();
      const pngBuffer = Buffer.from(b64Original, 'base64');

      const result = await ajustarDigitalAoLimiteBase64(pngBuffer, {
  ...
  // --------------------------------------------------------
  // CASO 13: reduzirImagemPng deve conseguir reduzir dedo 8 abaixo de 512K (Wallet real)
  // --------------------------------------------------------
  (hasRealData ? test : test.skip)(
    'ajustarDigitalAoLimiteBase64 reduz dedo 8 real para ≤ 512K chars (limite Wallet)',
    async () => {
      const b64Original = fs.readFileSync(DEDO8_FILE, 'utf-8').trim();
      const pngBuffer = Buffer.from(b64Original, 'base64');
  ...
  (hasRealData ? test : test.skip)(
    ...
    async () => {
      ...
      for (const m of fingerMetas) {
        ...
        const result = await ajustarDigitalAoLimiteBase64(pngBuf, {
          maxBase64Length: SPEC_LIMIT,
