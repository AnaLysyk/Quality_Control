  ...
  return (
    <div className={cn("space-y-2", className)}>
      <p className="text-sm text-(--tc-text-muted,#6b7280)">
        <span className="font-semibold text-(--tc-text,#0b1a3c)">Contexto atual:</span> {contextLabel}
      </p>
      {chips.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {chips.map((chip) => (
            <span
              key={chip}
              className="inline-flex rounded-full border border-(--tc-border,#d7deea) bg-(--tc-surface,#ffffff) px-3 py-1.5 text-[11px] font-semibold tracking-[0.04em] text-(--tc-text,#0b1a3c)"
            >
              {chip}
            </span>
          ))}
