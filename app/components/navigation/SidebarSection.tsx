          ...
          {visibleItems.map((item, index) => {
            ...
            return (
              <Fragment key={item.id}>
                {showGroupLabel && (
                  <span className="block px-2 pb-0.5 pt-2.5 text-[10px] font-semibold uppercase tracking-widest text-white/30">
                    {item.group}
                  </span>
                )}
                <Link
                ...
                >
                  {createElement(getIcon(item.iconKey), { size: 13, className: "shrink-0 text-white/50" })}
