...
import { FiActivity, FiAlertTriangle, FiBriefcase, FiCalendar, FiSearch, FiUsers } from "react-icons/fi";
...
export default function VisaoGeralCompacta() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      Promise.all([fetchApi("/api/admin/audit-logs?limit=12", { cache: "no-store" }), fetchApi(selectedCompany ? `/api/admin/defeitos?company=${encodeURIComponent(selectedCompany)}` : "/api/admin/defeitos", { cache: "no-store" })])
        .then(async ([a, d]) => {
