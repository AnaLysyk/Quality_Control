...
const EXECUTAR_APROVACAO_VISUAL = process.env.EXECUTAR_APROVACAO_VISUAL === "true";
...
async function aprovarSolicitacaoComoRevisor(page: Page, reviewer: Reviewer, data: RequestData) {
  ...
  await firstVisible(
    [
      page.getByText(data.requesterEmail),
      page.getByText(data.requesterName),
      page.getByText(data.requestedUser),
    ],
    `solicitação criada para ${data.requesterEmail}`,
  );

  await page.getByText(data.requesterEmail).first().click().catch(async () => {
    await page.getByText(data.requesterName).first().click();
  });

  await delay(2000);

  await clickByText(page, [/aprovar/i, /aceitar/i], "botão aprovar/aceitar");

  const confirmButton = page.getByRole("button", { name: /confirmar|aprovar|aceitar/i }).last();
  if (await confirmButton.isVisible().catch(() => false)) {
    await confirmButton.click();
    await delay();
  }

  await expect(page.getByText(/aprovad|aceit|sucesso/i).first()).toBeVisible({ timeout: 25000 });
  await delay(3000);
}
...
test.describe("VISUAL - Solicitação de acesso com aprovação", () => {
  test.skip(
    !EXECUTAR_APROVACAO_VISUAL,
    "Cenário manual: configure EXECUTAR_APROVACAO_VISUAL=true e as credenciais dos revisores.",
  );
  ...
  for (const reviewer of reviewers) {
    test(`solicitação pública aprovada por ${reviewer.name}`, async ({ page }) => {
      ...
      expect(reviewer.email, `Informe o e-mail do revisor ${reviewer.name}`).toBeTruthy();
      expect(reviewer.password, `Informe a senha do revisor ${reviewer.name}`).toBeTruthy();
      ...
      const receivedEmail = await waitForEmail((item) => {
        const all = emailText(item);
        return (
          all.includes("Solicitação de acesso recebida") &&
          all.includes(data.requesterEmail) &&
          all.includes(data.profileLabel) &&
          all.includes("Definida com segurança no formulário")
        );
      });
      ...
      await expect(receivedEmailPage.getByText(data.profileLabel).first()).toBeVisible();
      await expect(receivedEmailPage.getByText(data.requesterEmail).first()).toBeVisible();
      await expect(receivedEmailPage.getByText(/Definida com segurança no formulário/i).first()).toBeVisible();

      const statusUrl = extractStatusUrl(receivedEmail);

      await validarConsultaPublica(page, statusUrl, data, /em análise|análise|pendente/i);
      ...
      await expect(approvedEmailPage.getByText(data.profileLabel).first()).toBeVisible();
