...
function collectFlags(paths) {
  ...
  for (const file of paths) {
    if (file.startsWith("data/") || file.startsWith("database/")) flags.add("data");
    if (/(^|\/)\.env(\.|$)/i.test(file)) flags.add("env");
    if (
      file.startsWith("app/api/auth/") ||
      file.startsWith("backend/auth/") ||
    ...
    if ((file.startsWith("tests/") || file.startsWith("tests-e2e/")) || file.startsWith("playwright")) flags.add("e2e");
    if (file.startsWith("docs/")) flags.add("docs");
    if (file.startsWith("app/") || file.startsWith("components/") || file.startsWith("backend/")) {
      flags.add("app");
    }
  }
