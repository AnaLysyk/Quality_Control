...
type Selected = { kind: "person" | "company" | "project" | "profile"; id: string } | null;
...
export default function RelationshipManagementClient() {
  ...
  const availableProjects = useMemo(
    () => base.projects.filter((project) => !companyId || project.companyId === companyId),
    [base.projects, companyId],
  );
  ...
  async function loadBase() {
    ...
    if (!response.ok) {
      setMessage({ type: "error", text: body.error || "Falha ao carregar contextos" });
      return;
    }
    setBase(body);
  ...
  async function openPerson(id: string) {
    ...
    try {
      ...
      const firstCompany = body.person.memberships?.[0]?.companyId || body.person.projectTeamAssignments?.[0]?.company?.id || base.companies[0]?.id || "";
  ...
  function toggle(kind: Selected["kind"], id: string) {
  ...
  async function removeAssignment() {
    ...
    try {
      ...
      setRemoveReason("");
      if (person) await openPerson(person.id);
      await search();
    } catch (error) {
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(56,189,248,.08),transparent_30%),linear-gradient(180deg,#fff,#f7faff)] px-4 pb-28 pt-5 text-slate-950 dark:bg-[radial-gradient(circle_at_top,rgba(56,189,248,.10),transparent_28%),linear-gradient(180deg,#020617,#07111f)] dark:text-white sm:px-7 lg:px-10">
      {message ? (
        <div className={`fixed bottom-7 left-1/2 z-[120] flex w-[min(620px,calc(100vw-2rem))] -translate-x-1/2 items-center gap-3 rounded-2xl border px-4 py-3 shadow-2xl backdrop-blur-xl ${message.type === "success" ? "border-emerald-300/40 bg-emerald-50/95 text-emerald-950 dark:bg-emerald-950/90 dark:text-emerald-50" : "border-rose-300/40 bg-rose-50/95 text-rose-950 dark:bg-rose-950/90 dark:text-rose-50"}`}>
          {message.type === "success" ? <FiCheckCircle className="h-5 w-5" /> : <FiX className="h-5 w-5" />}
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void search(); }} className="mx-auto mt-7 flex max-w-5xl flex-col gap-2 rounded-[1.7rem] border border-slate-200 bg-white/85 p-2 shadow-[0_18px_55px_rgba(15,23,42,.08)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/70 sm:flex-row">
          <select value={mode} onChange={(event) => setMode(event.target.value as SearchMode)} className="h-12 rounded-2xl bg-transparent px-4 text-sm font-bold outline-none sm:w-36">
            <option value="all">Todos</option><option value="people">Pessoas</option><option value="companies">Empresas</option><option value="projects">Projetos</option><option value="profiles">Perfis</option>
          </select>
          <div className="flex flex-1 items-center gap-3 rounded-2xl bg-slate-100/80 px-4 dark:bg-white/[0.05]">
            <FiSearch className="text-cyan-600 dark:text-cyan-300" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar nome, e-mail, empresa, projeto ou perfil..." className="h-12 min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none" />
          </div>
        ...
        <div className="mt-8 border-y border-slate-200 dark:border-white/10">
          ...
          <div className="divide-y divide-slate-200 dark:divide-white/10">
            {data.people.map((item) => {
              ...
              return <div key={`person:${item.id}`}><button type="button" onClick={() => void openPerson(item.id)} className="flex w-full items-center gap-4 py-5 text-left hover:bg-cyan-500/[0.035]"><FiUser className="text-cyan-600" /><div className="flex-1"><p className="font-black">{personName(item)}</p><p className="mt-1 text-xs text-slate-500">{roleLabel(item.globalRole ?? item.role)} · {item.email}</p></div>{open ? <FiChevronUp /> : <FiChevronDown />}</button>{open && person ? <div className="grid gap-5 pb-6 pl-9 lg:grid-cols-2"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Vínculos atuais</p>{activeAssignments.length === 0 ? <p className="mt-3 text-sm text-slate-500">Nenhum vínculo ativo.</p> : activeAssignments.map((assignment) => <div key={assignment.id} className="mt-3 flex items-center gap-3 border-b border-slate-200 pb-3 dark:border-white/10"><FiBriefcase /><div className="flex-1"><p className="font-bold">{assignment.project.name}</p><p className="text-xs text-slate-500">{companyName(assignment.company)} · {roleLabel(assignment.role)}</p></div>{data.permissions.canDelete && assignment.role !== "leader_tc" ? <button type="button" onClick={() => setRemoveTarget(assignment)} className="text-rose-500"><FiTrash2 /></button> : null}</div>)}</div><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Criar vínculo</p><div className="mt-3 grid gap-3"><select value={companyId} onChange={(event) => setCompanyId(event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-transparent px-3 text-sm font-bold dark:border-white/10"><option value="">Selecione a empresa</option>{base.companies.map((company) => <option key={company.id} value={company.id}>{companyName(company)}</option>)}</select><select value={projectId} onChange={(event) => setProjectId(event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-transparent px-3 text-sm font-bold dark:border-white/10"><option value="">Selecione o projeto</option>{availableProjects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setAssignmentRole("leader_tc")} className={`h-10 rounded-xl border text-xs font-black ${assignmentRole === "leader_tc" ? "border-cyan-500 bg-cyan-500/10" : "border-slate-200 dark:border-white/10"}`}>Líder TC</button><button type="button" onClick={() => setAssignmentRole("qa_tc")} className={`h-10 rounded-xl border text-xs font-black ${assignmentRole === "qa_tc" ? "border-cyan-500 bg-cyan-500/10" : "border-slate-200 dark:border-white/10"}`}>Usuário TC</button></div><button type="button" onClick={() => void createAssignment()} disabled={saving || !projectId} className="h-11 rounded-xl bg-slate-950 text-sm font-black text-white disabled:opacity-40 dark:bg-cyan-300 dark:text-slate-950">Confirmar vínculo</button></div></div></div> : null}</div>;
            })}

            {data.companies.map((company) => { const open = selected?.kind === "company" && selected.id === company.id; const assignments = data.assignments.filter((item) => item.company.id === company.id); return <div key={`company:${company.id}`}><button type="button" onClick={() => toggle("company", company.id)} className="flex w-full items-center gap-4 py-5 text-left hover:bg-cyan-500/[0.035]"><FiHome className="text-cyan-600" /><div className="flex-1"><p className="font-black">{companyName(company)}</p><p className="mt-1 text-xs text-slate-500">Empresa · {assignments.length} vínculos encontrados</p></div>{open ? <FiChevronUp /> : <FiChevronDown />}</button>{open ? <div className="pb-6 pl-9">{assignments.length ? assignments.map((assignment) => <p key={assignment.id} className="mt-2 text-sm font-bold">{assignment.user?.full_name || assignment.user?.name} · {assignment.project.name} · {roleLabel(assignment.role)}</p>) : <p className="text-sm text-slate-500">Nenhum vínculo localizado nesta busca.</p>}</div> : null}</div>; })}

            {data.projects.map((project) => { const open = selected?.kind === "project" && selected.id === project.id; const assignments = data.assignments.filter((item) => item.project.id === project.id); const leader = assignments.find((item) => item.role === "leader_tc"); return <div key={`project:${project.id}`}><button type="button" onClick={() => toggle("project", project.id)} className="flex w-full items-center gap-4 py-5 text-left hover:bg-cyan-500/[0.035]"><FiFolder className="text-cyan-600" /><div className="flex-1"><p className="font-black">{project.name}</p><p className="mt-1 text-xs text-slate-500">{companyName(project.company)} · Projeto</p></div>{open ? <FiChevronUp /> : <FiChevronDown />}</button>{open ? <div className="grid gap-3 pb-6 pl-9 md:grid-cols-2"><p className="text-sm font-bold">Líder: {leader?.user?.full_name || leader?.user?.name || "Não localizado"}</p><p className="text-sm font-bold">Usuários TC: {assignments.filter((item) => item.role === "qa_tc").length}</p></div> : null}</div>; })}

            {data.profiles.map((profile) => { const open = selected?.kind === "profile" && selected.id === profile.role; const people = data.people.filter((item) => String(item.globalRole ?? item.role ?? "user") === profile.role); return <div key={`profile:${profile.role}`}><button type="button" onClick={() => toggle("profile", profile.role)} className="flex w-full items-center gap-4 py-5 text-left hover:bg-cyan-500/[0.035]"><FiUsers className="text-cyan-600" /><div className="flex-1"><p className="font-black">{roleLabel(profile.role)}</p><p className="mt-1 text-xs text-slate-500">Perfil · {profile.count} pessoas</p></div>{open ? <FiChevronUp /> : <FiChevronDown />}</button>{open ? <div className="pb-6 pl-9">{people.map((item) => <button key={item.id} type="button" onClick={() => void openPerson(item.id)} className="mr-3 mt-2 inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-2 text-xs font-bold dark:border-white/10">{personName(item)} <FiArrowRight /></button>)}</div> : null}</div>; })}
          </div>
        </div>
      </section>
