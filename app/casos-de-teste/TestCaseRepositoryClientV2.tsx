...
type CaseType = "manual" | "automated" | "hybrid";
type CaseStatus = "draft" | "active" | "review" | "obsolete" | "archived";
type CasePriority = "low" | "medium" | "high" | "critical";
type AutomationStatus = "none" | "planned" | "pending" | "review" | "approved" | "linked" | "stable" | "broken" | "disabled";
type AutomationTool = "none" | "playwright" | "postman" | "api";
...
const CASE_TYPES: CaseType[] = ["manual", "automated", "hybrid"];
const CASE_STATUSES: CaseStatus[] = ["draft", "active", "review", "obsolete", "archived"];
const CASE_PRIORITIES: CasePriority[] = ["low", "medium", "high", "critical"];
const AUTOMATION_STATUSES: AutomationStatus[] = ["none", "planned", "pending", "review", "approved", "linked", "stable", "broken", "disabled"];
...
function getProjectValue(activeProject: unknown, keys: string[]) {
...
function getAutomationTool(record: TestCaseRecord): AutomationTool {
  if (record.automationLink?.specFile) return "playwright";
  const tags = record.testCase.tags.join(" ").toLowerCase();
  if (tags.includes("postman")) return "postman";
  if (tags.includes("api")) return "api";
  return "none";
}

function getAutomationToolLabel(record: TestCaseRecord) {
  const tool = getAutomationTool(record);
  if (tool === "playwright") return "Playwright";
  if (tool === "postman") return "Postman/API";
  if (tool === "api") return "API";
  if (record.testCase.automationStatus !== "none") return "A definir";
  return "—";
}

function coerceCaseType(value: string): CaseType {
  return CASE_TYPES.includes(value as CaseType) ? (value as CaseType) : "manual";
}

function coerceCaseStatus(value: string): CaseStatus {
  return CASE_STATUSES.includes(value as CaseStatus) ? (value as CaseStatus) : "active";
}

function coercePriority(value: string): CasePriority {
  return CASE_PRIORITIES.includes(value as CasePriority) ? (value as CasePriority) : "medium";
}

function coerceAutomationStatus(value: string): AutomationStatus {
  return AUTOMATION_STATUSES.includes(value as AutomationStatus) ? (value as AutomationStatus) : "none";
}
...
function formFromRecord(record: TestCaseRecord): CaseForm {
  const automationTool = getAutomationTool(record);
  return {
    title: record.testCase.title ?? "",
    type: coerceCaseType(record.testCase.type),
    priority: coercePriority(record.testCase.priority),
    status: coerceCaseStatus(record.testCase.status),
    automationStatus: coerceAutomationStatus(record.testCase.automationStatus),
    automationTool,
    suiteName: record.testCase.suiteName ?? "",
...
export default function TestCaseRepositoryClientV2({ initialCompanySlug, lockCompanyScope = false }: TestCaseRepositoryClientV2Props) {
  ...
  const selected = useMemo(() => items.find((item) => item.testCase.id === selectedId) ?? items[0] ?? null, [items, selectedId]);
  ...
  function openEditForm(record: TestCaseRecord) {
  ...
  async function markAsAutomatable(record: TestCaseRecord) {
    ...
    setSelectedId(updated.testCase.id);
  }
  ...
  async function saveAutomationLink(testCaseId: string, title: string) {
    if (form.automationTool !== "playwright" || !form.automationSpecFile.trim()) return;
    await fetchApi(`/api/test-cases/${encodeURIComponent(testCaseId)}/automation`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        specFile: form.automationSpecFile.trim(),
        testTitle: form.automationTestTitle.trim() || title,
  ...
  async function handleSave() {
    ...
    try {
      ...
      let saved: TestCaseRecord;
      if (editingId) {
        saved = await patchCase(editingId, payload);
      } else {
        ...
        saved = (await createResponse.json()) as TestCaseRecord;
      ...
      await saveAutomationLink(saved.testCase.id, saved.testCase.title);
      setFormOpen(false);
      setEditingId(null);
      await loadCases();
    ...
    <section data-testid="test-case-repository" className="flex min-h-[calc(100vh-7rem)] w-full min-w-0 flex-col gap-3 rounded-[24px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-3 text-[var(--tc-text,#0b1a3c)] shadow-sm">
      <header data-testid="test-case-header" className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-4 py-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => setImportOpen((current) => !current)} className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] px-3 text-xs font-bold text-[var(--tc-text,#0b1a3c)]">
      ...
      {importOpen ? (
        <div className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-3">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => setImportOpen(false)} className="rounded-full border border-[var(--tc-border,#d7deea)] px-3 py-1 text-xs font-bold">Fechar</button>
          </div>
      ...
      <article className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-3">
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_160px_180px_160px_180px_auto]">
          ...
          <button type="button" onClick={resetFilters} className="min-h-9 self-end rounded-lg border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] px-3 text-xs font-bold">Limpar</button>
        </div>
      </article>
        ...
        <div className="grid flex-1 items-start gap-3 xl:grid-cols-[280px_minmax(520px,1fr)_380px] 2xl:grid-cols-[320px_minmax(0,1fr)_440px]">
          <aside data-testid="test-case-suite-tree" className="max-h-[calc(100vh-17rem)] overflow-auto rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-3 xl:sticky xl:top-3">
            <div className="flex items-center justify-between gap-2">
              <div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--tc-text-muted,#6b7280)]">Suites / Pastas</p><p className="mt-1 text-xs text-[var(--tc-text-secondary,#4b5563)]">{items.length} caso(s) no filtro</p></div>
            ...
            <div className="mt-3 space-y-1">
              ...
              {suiteOptions.map(([id, label]) => <SuiteButton key={id} active={suiteFilter === id} label={label} count={items.filter((item) => (item.testCase.suiteId || item.testCase.suiteName) === id).length} onClick={() => setSuiteFilter(id)} />)}
            </div>
          </aside>
          ...
          <article data-testid="test-case-table" className="min-h-[520px] overflow-hidden rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)]">
            <div className="flex min-h-12 items-center justify-between gap-2 border-b border-[var(--tc-border,#d7deea)] px-3 py-2">
              <div><h2 className="text-sm font-black">Casos de teste ({items.length})</h2><p className="text-xs text-[var(--tc-text-secondary,#4b5563)]">{selectedBulkIds.length ? `${selectedBulkIds.length} selecionado(s)` : "Selecione um caso para ver detalhes."}</p></div>
            ...
            <div data-testid="test-case-list" className="max-h-[calc(100vh-20rem)] min-h-[460px] overflow-auto">
              {items.length === 0 ? <div className="m-3 rounded-2xl border border-dashed border-[var(--tc-border,#d7deea)] px-4 py-8 text-center text-sm text-[var(--tc-text-muted,#6b7280)]">Nenhum caso encontrado para os filtros atuais.</div> : (
                <table className="w-full min-w-[820px] border-separate border-spacing-0 text-left text-xs">
                  <thead className="sticky top-0 z-10 bg-[var(--tc-surface-2,#f8fafc)] text-[10px] uppercase tracking-[0.14em] text-[var(--tc-text-muted,#6b7280)]"><tr><th className="w-9 border-b border-[var(--tc-border,#d7deea)] px-3 py-2"></th><th className="border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Código</th><th className="min-w-64 border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Título</th><th className="border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Tipo</th><th className="border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Automação</th><th className="border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Prioridade</th><th className="border-b border-[var(--tc-border,#d7deea)] px-3 py-2">Último run</th></tr></thead>
                  <tbody>{items.map((record) => {
                    const active = selected?.testCase.id === record.testCase.id;
                    return <tr key={record.testCase.id} data-testid="test-case-row" onClick={() => setSelectedId(record.testCase.id)} className={`cursor-pointer transition ${active ? "bg-[rgba(239,0,1,0.08)] shadow-[inset_3px_0_0_var(--tc-accent,#ef0001)]" : "hover:bg-[var(--tc-surface-2,#f8fafc)]"}`}>
                      ...
                      <td className="whitespace-nowrap border-b border-[var(--tc-border,#d7deea)] px-3 py-2 align-top"><Badge>{AUTOMATION_LABEL[record.testCase.automationStatus] ?? record.testCase.automationStatus}</Badge><span className="ml-1 text-[11px] text-[var(--tc-text-muted,#6b7280)]">{getAutomationToolLabel(record)}</span></td>
            ...
            {selected ? (
              <div data-testid="test-case-detail">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--tc-accent,#ef0001)]">{selected.testCase.key}</p><h2 data-testid="test-case-detail-title" className="mt-1 text-lg font-black tracking-[-0.02em]">{selected.testCase.title}</h2><p className="mt-2 text-sm leading-5 text-[var(--tc-text-secondary,#4b5563)]">{selected.testCase.description || "Sem descrição detalhada."}</p></div>
                  <div className="flex gap-1"><button type="button" onClick={() => openEditForm(selected)} className="rounded-lg border border-[var(--tc-border,#d7deea)] p-2" aria-label="Editar caso"><FiEdit2 /></button><button type="button" onClick={() => void handleArchive(selected)} className="rounded-lg border border-[var(--tc-border,#d7deea)] p-2" aria-label="Arquivar caso"><FiArchive /></button></div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-1 border-b border-[var(--tc-border,#d7deea)]">{["case", "steps", "automation", "runs", "history"].map((tab) => <button key={tab} type="button" onClick={() => setDetailTab(tab as typeof detailTab)} className={`px-3 py-2 text-xs font-black ${detailTab === tab ? "border-b-2 border-[var(--tc-accent,#ef0001)] text-[var(--tc-accent,#ef0001)]" : "text-[var(--tc-text-muted,#6b7280)]"}`}>{tab === "case" ? "Caso" : tab === "steps" ? "Passos" : tab === "automation" ? "Automação" : tab === "runs" ? "Runs" : "Histórico"}</button>)}</div>
                {detailTab === "case" ? <DetailSection selected={selected} /> : null}
                {detailTab === "steps" ? <StepsSection steps={selected.steps} /> : null}
                {detailTab === "automation" ? <AutomationSection selected={selected} onMark={() => void markAsAutomatable(selected)} /> : null}
      ...
      {formOpen ? <CreateCaseModal form={form} editing={Boolean(editingId)} saving={saving} error={formError} onClose={() => { setFormOpen(false); setEditingId(null); }} onSave={() => void handleSave()} onChange={setForm} onStepChange={updateStep} /> : null}
    </section>
  );
}
...
function AutomationSection({ selected, onMark }: { selected: TestCaseRecord; onMark: () => void }) {
  return <section className="mt-4 space-y-3 text-sm">{selected.testCase.automationStatus === "none" ? <button type="button" onClick={onMark} className="w-full rounded-xl border border-[var(--tc-accent,#ef0001)] bg-[rgba(239,0,1,0.08)] px-3 py-2 text-sm font-black text-[var(--tc-accent,#ef0001)]">Marcar como automatizável</button> : null}<div className="rounded-xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-3"><p><strong>Status:</strong> {AUTOMATION_LABEL[selected.testCase.automationStatus] ?? selected.testCase.automationStatus}</p><p className="mt-1"><strong>Ferramenta:</strong> {getAutomationToolLabel(selected)}</p><p className="mt-1"><strong>Spec:</strong> {selected.automationLink?.specFile || "Ainda não vinculado"}</p><p className="mt-1"><strong>Command:</strong> {selected.automationLink?.command || "Referência não informada"}</p></div><Link href={`/automacoes/casos?testCaseId=${encodeURIComponent(selected.testCase.id)}`} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--tc-primary,#011848)] px-3 py-2 text-sm font-black text-white"><FiCode /> Abrir na automação</Link></section>;
}
...
function CreateCaseModal({ form, editing, saving, error, onClose, onSave, onChange, onStepChange }: { form: CaseForm; editing: boolean; saving: boolean; error: string | null; onClose: () => void; onSave: () => void; onChange: React.Dispatch<React.SetStateAction<CaseForm>>; onStepChange: (index: number, field: keyof DraftStep, value: string) => void }) {
  const showAutomation = form.type !== "manual" || form.automationStatus !== "none" || form.automationTool !== "none";
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-slate-950/60 p-4 backdrop-blur-sm"><div data-testid="test-case-create-modal" className="w-full max-w-5xl rounded-3xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--tc-accent,#ef0001)]">{editing ? "Editar caso" : "Novo caso"}</p><h2 className="mt-1 text-2xl font-black">{editing ? "Editar caso de teste" : "Criar caso de teste"}</h2><p className="mt-1 text-sm text-[var(--tc-text-secondary,#4b5563)]">Cadastre o caso funcional. Automação é opcional e não executa por aqui.</p></div><button type="button" onClick={onClose} className="rounded-xl border border-[var(--tc-border,#d7deea)] p-2"><FiX /></button></div>
    <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_1fr]">
      <section className="rounded-2xl border border-[var(--tc-border,#d7deea)] p-4"><h3 className="text-sm font-black">Geral</h3><div className="mt-3 grid gap-3"><Field testId="test-case-title-input" label="Título" value={form.title} onChange={(value) => onChange((current) => ({ ...current, title: value }))} /><Select label="Tipo" value={form.type} onChange={(value) => onChange((current) => ({ ...current, type: value as CaseType, automationStatus: value === "manual" ? current.automationStatus : current.automationStatus === "none" ? "planned" : current.automationStatus }))} options={[["manual", "Manual"], ["hybrid", "Híbrido"], ["automated", "Automatizado"]]} /><Select label="Prioridade" value={form.priority} onChange={(value) => onChange((current) => ({ ...current, priority: value as CasePriority }))} options={[["low", "Baixa"], ["medium", "Média"], ["high", "Alta"], ["critical", "Crítica"]]} /><Field label="Tags" value={form.tags} onChange={(value) => onChange((current) => ({ ...current, tags: value }))} placeholder="smoke, login, regressao" /></div></section>
    ...
    <section className="mt-4 rounded-2xl border border-[var(--tc-border,#d7deea)] p-4"><h3 className="text-sm font-black">Automação</h3><p className="mt-1 text-xs text-[var(--tc-text-muted,#6b7280)]">Use para marcar fila/vínculo técnico. Não executa script.</p><div className="mt-3 grid gap-3 lg:grid-cols-3"><Select label="Status" value={form.automationStatus} onChange={(value) => onChange((current) => ({ ...current, automationStatus: value as AutomationStatus, type: value === "none" ? "manual" : current.type === "manual" ? "hybrid" : current.type }))} options={[["none", "Não aplicável"], ["planned", "Automatizável"], ["pending", "Pendente"], ["review", "Em revisão"], ["linked", "Vinculado"], ["stable", "Estável"], ["broken", "Quebrado"]]} /><Select label="Ferramenta sugerida" value={form.automationTool} onChange={(value) => onChange((current) => ({ ...current, automationTool: value as AutomationTool }))} options={[["none", "Nenhuma"], ["playwright", "Playwright"], ["postman", "Postman/API"], ["api", "API customizada"]]} /></div>{showAutomation && form.automationTool === "playwright" ? <div className="mt-3 grid gap-3 lg:grid-cols-2"><Field label="Spec file" value={form.automationSpecFile} onChange={(value) => onChange((current) => ({ ...current, automationSpecFile: value }))} placeholder="tests/e2e/login.spec.ts" /><Field label="Test title" value={form.automationTestTitle} onChange={(value) => onChange((current) => ({ ...current, automationTestTitle: value }))} /><Field label="Project" value={form.automationProject} onChange={(value) => onChange((current) => ({ ...current, automationProject: value }))} /><Field label="Tags" value={form.automationTags} onChange={(value) => onChange((current) => ({ ...current, automationTags: value }))} placeholder="@smoke @login" /><TextArea label="Command de referência" value={form.automationCommand} onChange={(value) => onChange((current) => ({ ...current, automationCommand: value }))} /></div> : null}</section>
    {error ? <p className="mt-4 text-sm font-bold text-rose-700">{error}</p> : null}<div className="mt-5 flex justify-end gap-2 border-t border-[var(--tc-border,#d7deea)] pt-4"><button type="button" onClick={onClose} className="rounded-xl border border-[var(--tc-border,#d7deea)] px-4 py-2 text-sm font-bold">Cancelar</button><button type="button" data-testid="test-case-save-button" onClick={onSave} disabled={saving} className="rounded-xl bg-[var(--tc-primary,#011848)] px-4 py-2 text-sm font-black text-white disabled:opacity-60">{saving ? "Salvando..." : editing ? "Salvar alterações" : "Salvar caso"}</button></div>
  </div></div>;
}
