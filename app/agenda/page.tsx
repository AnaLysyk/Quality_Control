...
export default function AgendaPage() {
  ...
  return (
    <main className="min-h-[calc(100vh-6rem)] px-4 py-4 text-[#011848] dark:text-white sm:px-6 lg:px-8">
      <section className="mx-auto flex min-h-[calc(100vh-8rem)] max-w-7xl flex-col gap-4">
        <section className="rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-4 shadow-lg shadow-slate-900/5 dark:border-white/10 dark:bg-slate-950/70">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ef0001]">{currentCopy.eyebrow}</p>
              <h1 className="text-2xl font-black tracking-tight sm:text-3xl">{currentCopy.title}</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">{currentCopy.description}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {viewOptions.map((option) => {
                const Icon = option.icon;
                const selected = option.id === currentView;
                return <button key={option.id} type="button" onClick={() => setActiveView(option.id)} className={["inline-flex items-center gap-2 rounded-2xl border px-3 py-2 text-xs font-black transition", selected ? "border-[#ef0001]/40 bg-red-50 text-[#ef0001] dark:border-red-400/30 dark:bg-red-400/10" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50 dark:border-white/10 dark:bg-slate-900 dark:text-slate-300"].join(" ")}><Icon className="h-4 w-4" />{option.label}</button>;
              })}
              <button type="button" onClick={loadEvents} disabled={loadingEvents} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-600 dark:border-white/10 dark:bg-slate-900 dark:text-slate-300"><FiRefreshCw className={loadingEvents ? "h-4 w-4 animate-spin" : "h-4 w-4"} />Atualizar</button>
            </div>
          </div>

          <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto_auto]">
            <label className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-900"><FiSearch className="h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por empresa, projeto, pessoa ou reunião..." className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-slate-400" /></label>
            {currentView === "company" || currentView === "management" ? <label className="flex min-w-[240px] items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-900"><FiFilter className="h-4 w-4 text-slate-400" /><select value={selectedCompany} onChange={(event) => setSelectedCompany(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none">{agendaManager ? <option value="all">Todas visíveis</option> : null}{companyOptions.map((company) => <option key={company.slug} value={company.slug}>{company.label}</option>)}</select></label> : null}
            <div className="grid grid-cols-3 gap-2 text-center text-xs font-bold sm:grid-cols-6 lg:min-w-[520px]">
              {[["Visíveis", `${counters.visible}/${counters.total}`], ["Futuras", counters.planned], ["Realizadas", counters.done], ["Não realizadas", counters.notDone], ["Empresas", companiesCount || companyOptions.length], ["5 min", counters.reminder]].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-white/10 dark:bg-slate-900/80"><span className="block text-slate-500 dark:text-slate-400">{label}</span><strong className="mt-0.5 block text-sm text-[#011848] dark:text-white">{value}</strong></div>)}
            </div>
          </div>
        </section>

        {error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700 dark:border-red-400/25 dark:bg-red-400/10 dark:text-red-100">{error}</div> : null}

        <section className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
          <section className="flex min-h-[620px] flex-col rounded-[1.5rem] border border-slate-200/80 bg-white/90 shadow-lg shadow-slate-900/5 dark:border-white/10 dark:bg-slate-950/70">
            <div className="flex flex-col gap-3 border-b border-slate-200 p-4 dark:border-white/10 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ef0001]">Calendário do ano</p>
                <h2 className="text-2xl font-black capitalize">{monthTitle(visibleMonth)}</h2>
              </div>
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => changeMonth(-1)} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-black dark:border-white/10 dark:bg-slate-900">← Mês anterior</button>
                <button type="button" onClick={() => { const now = new Date(); setVisibleMonth(new Date(now.getFullYear(), now.getMonth(), 1)); setSelectedDay(todayKey()); }} className="rounded-2xl border border-[#ef0001]/30 bg-red-50 px-4 py-2 text-sm font-black text-[#ef0001] dark:bg-red-400/10">Hoje</button>
                <button type="button" onClick={() => changeMonth(1)} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-black dark:border-white/10 dark:bg-slate-900">Próximo mês →</button>
              </div>
            </div>

            <div className="grid grid-cols-7 border-b border-slate-200 text-center text-xs font-black uppercase tracking-[0.18em] text-slate-400 dark:border-white/10">
            ...
            <div className="grid flex-1 grid-cols-7 grid-rows-6 overflow-hidden">
              {monthDays.map((day) => {
                ...
                return (
                  <button key={day.key} type="button" onClick={() => { setSelectedDay(day.key); setExpandedEventId(null); }} className={["min-h-[98px] border-b border-r border-slate-200 p-2 text-left transition dark:border-white/10", day.inMonth ? "bg-white dark:bg-slate-950/40" : "bg-slate-50 text-slate-400 dark:bg-slate-900/30", selected ? "ring-2 ring-inset ring-[#ef0001]" : "hover:bg-red-50/60 dark:hover:bg-red-400/10"].join(" ")}>
                    ...
                    <div className="mt-2 space-y-1">
                      ...
                      {dayEvents.length > 3 ? <span className="block rounded-lg bg-slate-100 px-2 py-1 text-[11px] font-bold text-slate-500 dark:bg-slate-900">+{dayEvents.length - 3} outros</span> : null}
                    </div>
                  </button>
                );
              })}
          ...
          <aside className="min-h-[620px] rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-4 shadow-lg shadow-slate-900/5 dark:border-white/10 dark:bg-slate-950/70">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ef0001]">Dia selecionado</p>
                <h3 className="text-xl font-black">{formatDay(selectedDay)}</h3>
              </div>
              <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-black dark:border-white/10 dark:bg-slate-900">{selectedDayEvents.length}</span>
            </div>

            <div className="grid gap-3">
              {selectedDayEvents.map((event) => {
                const expanded = expandedEventId === event.id;
                return <article key={event.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900/60"><button type="button" onClick={() => setExpandedEventId(expanded ? null : event.id)} className="w-full p-3 text-left"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="flex items-center gap-1 text-xs font-bold text-slate-500"><FiClock className="text-[#ef0001]" />{formatTime(event.startAt)} - {formatTime(event.endAt)} · {duration(event.startAt, event.endAt)}</p><h4 className="mt-1 truncate text-sm font-black">{event.title}</h4><p className="mt-1 text-xs text-slate-500">{event.companyName || event.companySlug || contextLabel(event.context)}</p></div><span className={["shrink-0 rounded-full border px-2 py-1 text-[10px] font-black", statusClass(event.status)].join(" ")}>{statusLabel(event.status)}</span></div></button>{expanded ? <div className="border-t border-slate-200 p-3 text-sm dark:border-white/10"><p className="text-slate-600 dark:text-slate-300">{event.description || event.releaseName || "Evento de agenda operacional."}</p><div className="mt-3 grid gap-2 rounded-2xl bg-slate-50 p-3 text-xs dark:bg-slate-950/60"><span><strong>Projeto:</strong> {event.projectSlug || "Sem projeto"}</span><span><strong>Responsável:</strong> {event.ownerName || "Sem responsável"}</span><span><strong>Participantes:</strong> {(event.participantNames?.length ? event.participantNames : ["Não informados"]).join(", ")}</span><span className="inline-flex items-center gap-1"><FiMessageCircle className="text-[#ef0001]" />Registro vinculado ao Chat/Brain quando a reunião nascer de conversa.</span></div>{canUpdateStatus ? <div className="mt-3 flex flex-wrap gap-2"><button onClick={() => updateStatus(event.id, "planned")} className="rounded-xl border border-blue-200 bg-blue-50 px-2 py-1 text-xs font-black text-blue-700">Vai acontecer</button><button onClick={() => updateStatus(event.id, "done")} className="rounded-xl border border-emerald-200 bg-emerald-50 px-2 py-1 text-xs font-black text-emerald-700">Realizada</button><button onClick={() => updateStatus(event.id, "cancelled")} className="rounded-xl border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-black text-slate-600">Não realizada</button><button onClick={() => updateStatus(event.id, "blocked")} className="rounded-xl border border-red-200 bg-red-50 px-2 py-1 text-xs font-black text-red-700">Bloquear</button></div> : null}</div> : null}</article>;
              })}

              {!loadingEvents && selectedDayEvents.length === 0 ? <div className="rounded-[1.5rem] border border-dashed border-slate-300 p-8 text-center text-slate-500 dark:border-white/15"><FiCheckCircle className="mx-auto h-7 w-7 text-emerald-500" /><p className="mt-3 font-bold">Nenhum agendamento neste dia.</p><p className="mt-1 text-sm">Troque o mês, selecione outro dia ou ajuste os filtros.</p></div> : null}
            </div>
          </aside>
        </section>
      </section>
