    ...
    <main className="space-y-5">
      ...
      <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-8">
          ...
          <div className="space-y-1"><FieldLabel>Contexto</FieldLabel><select value={filters.context} onChange={(event) => setFilters((current) => ({ ...current, context: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold"><option value="">Todos</option>{CONTEXT_OPTIONS.map((context) => <option key={context} value={context}>{contextLabel(context)}</option>)}</select></div>
          <div className="space-y-1"><FieldLabel>Perfil</FieldLabel><select value={filters.audienceProfile} onChange={(event) => setFilters((current) => ({ ...current, audienceProfile: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold"><option value="">Todos</option>{AUDIENCE_PROFILE_OPTIONS.map((profile) => <option key={profile} value={profile}>{audienceLabel(profile)}</option>)}</select></div>
          ...
          <div className="mt-4 space-y-3">
            {selectedDayEvents.length ? selectedDayEvents.map((event) => (
              <article key={event.id} className="rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-2,#f8fafc) p-4">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-(--tc-text-muted,#6b7280)">{formatDate(event.startAt)} → {formatDate(event.endAt)}</p><h3 className="mt-1 text-base font-black text-(--tc-text,#0b1a3c)">{event.title}</h3></div><select value={event.status} disabled={updatingId === event.id} onChange={(changeEvent) => void handleStatusChange(event.id, changeEvent.target.value)} className="rounded-full border border-(--tc-border,#d7deea) bg-white px-2.5 py-1 text-[11px] font-bold">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></div>
          ...
          <section className="rounded-[28px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#fff) p-5 shadow-sm">
            ...
            <form className="mt-4 space-y-4" onSubmit={handleCreateEvent}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1"><FieldLabel>Contexto</FieldLabel><select value={form.context} onChange={(event) => setForm((current) => ({ ...current, context: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold">{CONTEXT_OPTIONS.map((context) => <option key={context} value={context}>{contextLabel(context)}</option>)}</select></div>
                ...
                <div className="space-y-1"><FieldLabel>Inicio</FieldLabel><input type="datetime-local" value={form.startAt} onChange={(event) => setForm((current) => ({ ...current, startAt: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
                <div className="space-y-1"><FieldLabel>Fim</FieldLabel><input type="datetime-local" value={form.endAt} onChange={(event) => setForm((current) => ({ ...current, endAt: event.target.value }))} className="w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" /></div>
                <div className="space-y-1"><FieldLabel>Perfis que enxergam</FieldLabel><textarea value={form.audienceProfiles} onChange={(event) => setForm((current) => ({ ...current, audienceProfiles: event.target.value }))} className="min-h-24 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="leader_tc, technical_support" /></div>
                <div className="space-y-1"><FieldLabel>Participantes</FieldLabel><textarea value={form.participantNames} onChange={(event) => setForm((current) => ({ ...current, participantNames: event.target.value }))} className="min-h-24 w-full rounded-xl border border-(--tc-border,#d7deea) bg-white px-3 py-2 text-sm font-semibold" placeholder="Ana, Suporte" /></div>
