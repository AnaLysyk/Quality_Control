...
import { listTicketComments } from "@/backend/ticketCommentsStore";
...
export async function toolSummarizeEntity(user: AuthUser, context: AssistantScreenContext, message: string): Promise<AssistantExecutorResult> {
  ...
  // â”€â”€â”€ Resumir perfil do usuário â”€â”€â”€
  if (normalized.includes("perfil") || normalized.includes("meus dados") || normalized.includes("meu usuario") || normalized.includes("meu usuário")) {
    ...
    replyParts.push(
      "",
      "---",
      "ðŸ’¡ Posso usar esse contexto para estruturar chamados, comentários ou outras ações."
    );
  ...
  if (ticket) {
    ...
    const replyParts = [
      ...
      `| **Responsável** | ${ticket.assignedToName ?? "âš ï¸ não definido"} |`,
  ...
  if (context.module === "company" || normalized.includes("empresa")) {
    ...
    if (current) {
      ...
      replyParts.push(
        "",
        "---",
        "ðŸ’¡ Posso buscar tickets, usuários ou gerar relatórios desta empresa."
      );
