...
import { FiActivity, FiExternalLink, FiMove, FiRefreshCw, FiSave, FiSearch, FiX, FiZap } from "react-icons/fi";
...
export function BrainNodeOverlay({ node, nodes, edges, onClose, onResetFocus, onOpenRelatedModule, debugMode = false, onBackNode, canBackNode = false }: BrainNodeOverlayProps) {
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <section className="qc-brain-rag-context-card mt-3 rounded-2xl border border-cyan-100/14 bg-cyan-100/[0.055] p-3">
            ...
            {ragLoading ? <p className="mt-3 text-xs font-bold text-slate-200/78">Consultando cérebro, permissões, eventos, notas e documentos vinculados...</p> : ragContext ? (
              <div className="mt-3 grid gap-3">
                ...
                {ragContext.memory?.length ? <div className="rounded-xl border border-white/10 bg-black/18 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-100/45">Memórias encontradas</p><div className="mt-2 grid gap-2">{ragContext.memory.slice(0, 4).map((item) => <div key={item.id} className="rounded-lg border border-white/10 bg-white/[0.035] px-3 py-2"><p className="text-xs font-black text-slate-50">{item.title ?? item.kind ?? "Memória"}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{item.kind ?? "contexto"}{item.status ? ` · ${item.status}` : ""}</p></div>)}</div></div> : <p className="rounded-xl border border-white/10 bg-black/18 p-3 text-xs font-bold text-slate-200/72">Nenhuma memória salva para este nó ainda. Use o campo abaixo para ensinar a regra real, ou pergunte ao Brain Chat para cruzar logs, documentos e relações.</p>}
              </div>
          ...
          <section className="mt-3 rounded-2xl border border-cyan-100/14 bg-slate-950/40 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/45">Ensinar uma memória útil</p>
            <input value={memoryTitle} onChange={(event) => setMemoryTitle(event.target.value)} placeholder="Ex.: regra real desta tela, permissão ou exceção" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-slate-500 focus:border-cyan-200/40" />
            <textarea value={memorySummary} onChange={(event) => setMemorySummary(event.target.value)} placeholder="Escreva a regra, vínculo, exceção ou contexto que deve ser lembrado neste nó." rows={3} className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-bold text-white outline-none placeholder:text-slate-500 focus:border-cyan-200/40" />
        ...
        <footer className="flex items-center justify-between gap-2 border-t border-white/10 bg-slate-950/72 p-3">
          ...
          <button type="button" onClick={onResetFocus} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-black text-white/70 hover:border-cyan-100/35 hover:text-white">Núcleo Brain</button>
