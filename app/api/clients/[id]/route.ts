...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!company) return jsonError("Empresa nÃ£o encontrada", 404);
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!current) return jsonError("Empresa nÃ£o encontrada", 404);
  ...
  if (!nextName) return jsonError("Nome da empresa obrigatÃ³rio", 400);
  ...
  if (duplicateByName) return jsonError("Empresa jÃ¡ cadastrada com esse nome", 409);
  ...
  const duplicateByTaxId =
    nextTaxId.length > 0
      ...
      : null;
  if (duplicateByTaxId) return jsonError("CNPJ jÃ¡ cadastrado para outra empresa", 409);

  const { nextProjectCodes, patch } = buildCompanyUpdatePatch(input, current);
  const updated = await updateLocalCompany(id, patch);

  if (!updated) return jsonError("Empresa nÃ£o encontrada", 404);
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (!deleted) return jsonError("Empresa nÃ£o encontrada", 404);
