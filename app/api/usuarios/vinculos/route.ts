...
export async function POST(req: Request) {
  ...
  try {
    const result = await db.$transaction(async (tx) => {
      ...
      const membershipRole = body.role === "leader_tc" ? Role.leader_tc : Role.user;
      const membership = await tx.membership.upsert({
        where: { userId_companyId: { userId: target.id, companyId: company.id } },
        update: body.role === "leader_tc" ? { role: membershipRole } : {},
        create: {
          userId: target.id,
      ...
      if (body.role === "qa_tc" && !membership.allowedProjectIds.includes(project.id)) {
