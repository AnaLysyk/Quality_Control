...
const EMPTY_SEARCH: SearchResponse = {
  companies: [], projects: [], people: [], assignments: [], profiles: [],
  permissions: { canCreate: false, canEdit: false, canDelete: false },
};
const PAGE_SIZE = 5;
const ROLE_LABEL: Record<string, string> = {
  leader_tc: "Líder TC", qa_tc: "Usuário TC", testing_company_user: "Usuário TC",
  technical_support: "Suporte Técnico", support: "Suporte Técnico", company_admin: "Empresa",
  empresa: "Empresa", company: "Empresa", company_user: "Usuário empresarial", user: "Usuário empresarial",
};
...
function Row({ icon, title, subtitle, open, onClick, children }: {
  icon: ReactNode; title: string; subtitle: string; open: boolean; onClick: () => void; children?: ReactNode;
}) {
  return (
...
export default function RelationshipManagementClient() {
  ...
  async function openPerson(id: string) {
    if (selected?.kind === "person" && selected.id === id) { setSelected(null); setPerson(null); return; }
  ...
  return (
    <main className="relationship-shell">
      {message ? <div className={`fixed bottom-6 left-1/2 z-[120] flex w-[min(620px,calc(100vw-2rem))] -translate-x-1/2 items-center gap-3 rounded-2xl border px-4 py-3 shadow-2xl backdrop-blur-xl ${message.type === "success" ? "border-emerald-300/40 bg-emerald-50/95 text-emerald-950 dark:bg-emerald-950/90 dark:text-emerald-50" : "border-rose-300/40 bg-rose-50/95 text-rose-950 dark:bg-rose-950/90 dark:text-rose-50"}`}>{message.type === "success" ? <FiCheckCircle /> : <FiX />}<p className="flex-1 text-sm font-bold">{message.text}</p><button type="button" onClick={() => setMessage(null)}><FiX /></button></div> : null}
      <section className="relationship-stage">
        ...
        <section className="relationship-results">
          ...
          {results.length > PAGE_SIZE ? <nav className="relationship-pagination" aria-label="Paginação dos resultados"><button type="button" className="relationship-page-button" disabled={page === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}><FiArrowLeft /></button>{Array.from({ length: pageCount }, (_, index) => index + 1).map((pageNumber) => <button key={pageNumber} type="button" className="relationship-page-button" data-active={pageNumber === page} onClick={() => setPage(pageNumber)}>{pageNumber}</button>)}<button type="button" className="relationship-page-button" disabled={page === pageCount} onClick={() => setPage((current) => Math.min(pageCount, current + 1))}><FiArrowRight /></button></nav> : null}
        </section>
      </section>

      {historyOpen ? <div className="fixed inset-0 z-[110] bg-slate-950/35 backdrop-blur-sm" onClick={() => setHistoryOpen(false)}><aside className="absolute bottom-0 right-0 top-0 w-[min(430px,92vw)] overflow-y-auto border-l bg-white p-6 shadow-2xl dark:bg-slate-950" style={{ borderColor: "var(--rel-line)" }} onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Histórico</p><h2 className="mt-1 text-xl font-black">Alterações de vínculo</h2></div><button type="button" onClick={() => setHistoryOpen(false)}><FiX /></button></div><div className="mt-6 divide-y" style={{ borderColor: "var(--rel-line)" }}>{historyAssignments.length === 0 ? <p className="py-8 text-sm" style={{ color: "var(--rel-muted)" }}>Selecione uma pessoa para visualizar o histórico.</p> : historyAssignments.map((assignment) => <div key={assignment.id} className="py-4"><p className="font-bold">{assignment.project.name}</p><p className="mt-1 text-xs" style={{ color: "var(--rel-muted)" }}>{companyName(assignment.company)} · {roleLabel(assignment.role)}</p><p className="mt-2 text-xs" style={{ color: "var(--rel-muted)" }}>{assignment.removalReason || "Sem justificativa registrada"}</p></div>)}</div></aside></div> : null}

      {removeTarget ? <div className="fixed inset-0 z-[115] grid place-items-center bg-slate-950/45 px-4 backdrop-blur-sm"><div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-950"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Remover vínculo</h2><button type="button" onClick={() => setRemoveTarget(null)}><FiX /></button></div><p className="mt-2 text-sm" style={{ color: "var(--rel-muted)" }}>{removeTarget.project.name} · {companyName(removeTarget.company)}</p><textarea value={removeReason} onChange={(event) => setRemoveReason(event.target.value)} placeholder="Informe a justificativa" className="mt-5 min-h-28 w-full rounded-2xl border bg-transparent p-4 text-sm outline-none" style={{ borderColor: "var(--rel-line)" }} /><div className="mt-4 flex justify-end gap-3"><button type="button" onClick={() => setRemoveTarget(null)} className="rounded-xl px-4 py-2 text-sm font-bold">Cancelar</button><button type="button" onClick={() => void removeAssignment()} disabled={saving} className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-black text-white disabled:opacity-50">Confirmar remoção</button></div></div></div> : null}
    </main>
  );
}
