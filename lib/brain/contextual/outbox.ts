import { sanitizeBrianImpulse } from "./governance";
import { stableId } from "./normalizer";
import type { BrianDeadLetterImpulse, BrianImpulseEnvelope, BrianOutboxRecord } from "./types";

export function buildBrianIdempotencyKey(impulse: BrianImpulseEnvelope) {
  return stableId("brian_idempotency", [impulse.source, impulse.subject, impulse.type, impulse.id]);
}

export function createBrianOutboxRecord(
  impulse: BrianImpulseEnvelope,
  options?: { now?: string; maxRetries?: number },
): BrianOutboxRecord {
  const now = options?.now ?? new Date().toISOString();
  const sanitized = sanitizeBrianImpulse(impulse);
  const idempotencyKey = buildBrianIdempotencyKey(sanitized.impulse);
  return {
    id: stableId("brian_outbox", [idempotencyKey]),
    idempotencyKey,
    status: "pending",
    impulse: sanitized.impulse,
    retryCount: 0,
...
export function createBrianDeadLetterImpulse(input: {
  impulse: BrianImpulseEnvelope;
  error: unknown;
  retryCount?: number;
  now?: string;
}): BrianDeadLetterImpulse {
  const now = input.now ?? new Date().toISOString();
  const message = input.error instanceof Error ? input.error.message : String(input.error);
  return {
    id: stableId("brian_dead_letter", [input.impulse.id, now]),
    impulseId: input.impulse.id,
    type: input.impulse.type,
    payload: sanitizeBrianImpulse(input.impulse).impulse,
