...
export default function AutomacoesCasosPage() {
  ...
  const companyOptions = useMemo(
    () => clients.map((company) => ({ name: company.name, slug: company.slug })),
    [clients],
  );
  ...
  return (
    <section data-testid="automation-context" className="space-y-4 rounded-[28px] border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-5 shadow-sm">
      <header className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--tc-accent,#ef0001)]">Vínculo técnico de automação</p>
            <h1 className="mt-2 text-2xl font-black tracking-[-0.03em] text-[var(--tc-text,#0b1a3c)]">
            ...
            <p className="mt-2 text-sm text-[var(--tc-text-secondary,#4b5563)]">
              O caso continua único no repositório. Esta tela só cria ou ajusta o vínculo Playwright/Postman/API. Não executa automação.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Link href="/automacoes/casos" className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-[var(--tc-border,#d7deea)] bg-white px-4 py-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">
              <FiArrowLeft className="h-4 w-4" />
              Voltar para fila
            </Link>
            <button type="button" data-testid="automation-save-link-button" onClick={() => void handleSaveLink()} disabled={saving || loading || !linkedCase || !specFile.trim()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-[var(--tc-primary,#011848)] px-5 py-2 text-sm font-semibold text-white disabled:opacity-60">
      ...
      <div data-testid="automation-linked-test-case" className="grid gap-3 rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,0.8fr)]">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--tc-text-muted,#6b7280)]">Caso vinculado</p>
          <p className="mt-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">{linkedCase ? `${linkedCase.key} - ${linkedCase.title}` : "Caso indisponível"}</p>
          <p className="mt-2 text-sm leading-6 text-[var(--tc-text-secondary,#4b5563)]">{linkedCase?.description || "Sem descrição detalhada."}</p>
        </div>
        <div className="space-y-2 text-sm text-[var(--tc-text,#0b1a3c)]">
          ...
          <Link href={`/casos-de-teste?case=${encodeURIComponent(testCaseId)}`} className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--tc-accent,#ef0001)]">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
        <main className="space-y-4">
          <section data-testid="automation-panel" className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white p-4">
            <div className="flex items-center gap-2">
              <FiCode className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" />
              <h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">Vínculo Playwright</h2>
            </div>
            <div className="mt-4 grid gap-3 lg:grid-cols-2">
              ...
              <Field testId="automation-spec-file-input" label="Spec file" value={specFile} onChange={setSpecFile} placeholder="tests-e2e/auth/login.spec.ts" />
              <Field testId="automation-test-title-input" label="Test title" value={testTitle} onChange={setTestTitle} placeholder="deve validar login com usuário válido" />
              <Field label="test.describe" value={testDescribe} onChange={setTestDescribe} placeholder="Autenticação" />
              <SelectField testId="automation-project-select" label="Project" value={playwrightProject} onChange={setPlaywrightProject} options={["chromium", "edge", "firefox", "webkit"]} />
            ...
            <label className="mt-3 grid gap-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">
              Command de referência
              <textarea value={command} onChange={(event) => setCommand(event.target.value)} placeholder={suggestedCommand || "npx playwright test ..."} className="min-h-24 rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-4 py-3 text-sm outline-none transition focus:border-[var(--tc-accent,#ef0001)]" />
            </label>
            <div data-testid="automation-command-preview" className="mt-3 rounded-2xl border border-dashed border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] px-4 py-3 text-sm text-[var(--tc-text,#0b1a3c)]">
        ...
        <aside className="space-y-4">
          <section className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
            <div className="flex items-center gap-2"><FiCpu className="h-4 w-4 text-[var(--tc-accent,#ef0001)]" /><h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">Mapeamento do caso</h2></div>
            <div className="mt-3 space-y-3">
              {steps.length > 0 ? steps.map((step) => <div key={step.id} className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white p-3 text-sm text-[var(--tc-text,#0b1a3c)]"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--tc-accent,#ef0001)]">Passo {step.order}</p><p className="mt-2"><strong>Ação:</strong> {step.action}</p><p className="mt-1"><strong>Esperado:</strong> {step.expectedResult}</p></div>) : <p className="text-sm text-[var(--tc-text-secondary,#4b5563)]">O caso ainda não possui passos cadastrados.</p>}
            </div>
          </section>
          <section className="rounded-2xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface-2,#f8fafc)] p-4">
            <h2 className="text-sm font-black text-[var(--tc-text,#0b1a3c)]">Última execução conhecida</h2>
            <div className="mt-3 space-y-2 text-sm text-[var(--tc-text,#0b1a3c)]">
              ...
              <p className="text-[var(--tc-text-muted,#6b7280)]">Execuções reais ficam nas telas de execução/CI, não neste formulário.</p>
            </div>
          </section>
        </aside>
      </div>
...
function Field({ label, value, onChange, placeholder, testId }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; testId?: string }) {
  return <label className="grid gap-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">{label}<input data-testid={testId} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="min-h-11 rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-4 text-sm outline-none transition focus:border-[var(--tc-accent,#ef0001)]" /></label>;
}
...
function SelectField({ label, value, onChange, options, testId }: { label: string; value: string; onChange: (value: string) => void; options: string[]; testId?: string }) {
  return <label className="grid gap-2 text-sm font-semibold text-[var(--tc-text,#0b1a3c)]">{label}<select data-testid={testId} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 rounded-2xl border border-[var(--tc-border,#d7deea)] bg-white px-4 text-sm outline-none transition focus:border-[var(--tc-accent,#ef0001)]">{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}
