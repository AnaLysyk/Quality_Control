...
export async function PATCH(req: Request) {
  try {
    ...
    if (!updated) {
      return NextResponse.json({ error: "NÃ£o foi possÃ­vel atualizar" }, { status: 500 });
    }
    ...
    const message = error instanceof Error && error.message.trim() ? error.message.trim() : "NÃ£o foi possÃ­vel atualizar os dados";
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!canDeleteDirectly) {
      return NextResponse.json({ error: "Somente lider TC ou suporte tÃ©cnico podem deletar o perfil diretamente" }, { status: 403 });
    }
    ...
    if (!updated) {
      return NextResponse.json({ error: "NÃ£o foi possÃ­vel deletar o usuÃ¡rio" }, { status: 500 });
    }

    return NextResponse.json({ ok: true }, { status: 200 });
  } catch (error) {
    const message = error instanceof Error && error.message.trim() ? error.message.trim() : "NÃ£o foi possÃ­vel deletar o usuÃ¡rio";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
