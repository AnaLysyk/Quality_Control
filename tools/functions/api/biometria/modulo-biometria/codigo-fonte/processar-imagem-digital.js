...
const logger = require('./registrar-logs');
const {
  MAX_BASE64_LENGTH,
  SPEC_WIDTH,
  SPEC_HEIGHT,
 ...
 * @returns {string}
 */
function converterBufferParaBase64(imageBuffer) {
 ...
 * @returns {number}
 */
function estimarTamanhoBase64(byteLength) {
 ...
 * @returns {number}
 */
function calcularMaximoBytesParaBase64(maxBase64Chars) {
 ...
 */
async function reduzirImagemPng(originalBuffer, currentWidth, currentHeight, attempt) {
 ...
 */
async function aumentarImagemAteTamanhoAlvo(originalBuffer, originalWidth, originalHeight, targetBase64Length, maxAttempts = 10, contextId = 'unknown') {
  // Tentar primeiro sem compressão
  let currentBuffer = await sharp(originalBuffer)
    .png({ compressionLevel: 0, adaptiveFiltering: false })
    .toBuffer();

  let base64 = converterBufferParaBase64(currentBuffer);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    base64 = converterBufferParaBase64(currentBuffer);
 ...
 */
async function ajustarDigitalAoLimiteBase64(imageBuffer, options = {}) {
  const {
  ...
  if (ehFormatoWsq(imageBuffer)) {
    const wsqBase64 = converterBufferParaBase64(imageBuffer);
  ...
  const originalBase64Raw = converterBufferParaBase64(imageBuffer);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    currentBuffer = await reduzirImagemPng(
