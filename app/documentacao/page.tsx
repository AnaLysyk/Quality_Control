...
export default function DocumentationPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--tc-page-bg,#f5f7fb)] px-4 py-6 text-[var(--tc-text,#0b1a3c)] lg:px-8">
      <section className="mx-auto max-w-7xl space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[260px_1fr]">
          <aside className="rounded-3xl border border-[var(--tc-border,#dfe5f1)] bg-[var(--tc-surface,#fff)] p-3 shadow-sm">
            {areas.map((area) => (
              <button
                key={area}
                type="button"
                ...
                className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-semibold transition ${
                  activeArea === area ? "bg-[var(--tc-accent,#ef0001)] text-white" : "text-[var(--tc-text,#0b1a3c)] hover:bg-[var(--tc-surface-2,#eef4ff)]"
                }`}
              >
          ...
          <section className="grid gap-4 md:grid-cols-2">
            {filtered.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-[var(--tc-border,#dfe5f1)] bg-[var(--tc-surface,#fff)] p-8 text-sm text-[var(--tc-text-muted,#64748b)] md:col-span-2">
                Nenhum documento encontrado para a busca atual.
              </div>
            ) : (
              filtered.map((entry) => (
                <article key={entry.id} className="rounded-3xl border border-[var(--tc-border,#dfe5f1)] bg-[var(--tc-surface,#fff)] p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <span className="rounded-full bg-[var(--tc-accent-soft,#fee2e2)] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[var(--tc-accent,#ef0001)]">
                      {entry.area}
                    </span>
                    <span className="text-xs text-[var(--tc-text-muted,#64748b)]">{entry.updatedAt}</span>
                  </div>
                  <h2 className="mt-4 text-xl font-black">{entry.title}</h2>
                  <p className="mt-2 text-sm font-semibold text-[var(--tc-text-secondary,#475569)]">{entry.summary}</p>
                  <p className="mt-4 text-sm leading-6 text-[var(--tc-text-muted,#64748b)]">{entry.content}</p>
                </article>
              ))
            )}
          </section>
