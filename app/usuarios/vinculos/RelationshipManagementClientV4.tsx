...
type Company = { id: string; name: string; company_name?: string | null; slug: string; logo_url?: string | null };
...
type Assignment = {
  ...
  user?: { id: string; name: string; full_name?: string | null; email: string; avatar_url?: string | null; avatar_key?: string | null };
};
...
type Person = {
  ...
  links?: Array<{ companyId: string; company: Company }>;
...
const avatarSource = (person?: Person | Assignment["user"] | null) => person?.avatar_url || person?.avatar_key || null;

function Row({ leading, title, subtitle, badge, open, onClick, children }: {
  leading: ReactNode;
  title: string;
  subtitle: string;
  badge: string;
  ...
  return (
    <div className="relationship-row" data-open={open}>
      <button type="button" className="relationship-row-trigger" onClick={onClick}>
        <span className="relationship-row-avatar">{leading}</span>
...
export default function RelationshipManagementClientV4() {
  ...
  async function search(searchQuery = query.trim(), searchMode = mode) {
    ...
    try {
      ...
      const response = await fetch(`/api/usuarios/vinculos/search-v2?${params.toString()}`, { cache: "no-store", signal: controller.signal });
      const body = await response.json();
  ...
  async function addQaProject() {
    ...
    try {
      ...
      showMessage("success", "Projeto vinculado ao Usuário TC.");
      setAddOpen(false);
  ...
  async function removeQaProject() {
    ...
    try {
      ...
      showMessage("success", "Vínculo removido e histórico preservado.");
  ...
  function renderPerson(item: Extract<ResultItem, { kind: "person" }>) {
    ...
    const membershipCount = item.person.memberships?.length || item.person.links?.length || (item.person.home_company_id || item.person.created_by_company_id ? 1 : 0);
    const projectCount = item.person.projectTeamAssignments?.length ?? 0;

    return (
      <Row
        key={item.id}
        leading={<UserAvatar src={avatarSource(item.person)} name={personName(item.person)} size="sm" />}
      ...
      >
        {person ? mode === "leaders" ? (
          ...
          <BusinessUserManagementPanel person={person} onChanged={refreshPersonAndSearch} onMessage={showMessage} />
        ) : null}
      </Row>
    );
  }
  ...
  function renderCompany(item: Extract<ResultItem, { kind: "company" }>) {
    ...
    return (
      <Row
        key={item.id}
        leading={<span className="relationship-company-avatar"><FiHome /></span>}
      ...
      >
        <div className="relationship-company-context">
          ...
          {assignments.length ? assignments.map((assignment) => (
            <div key={assignment.id} className="relationship-link-line">
              <UserAvatar src={avatarSource(assignment.user)} name={personName(assignment.user)} size="sm" />
              <div className="flex-1">
  ...
  return (
    <main className="relationship-shell relationship-shell-v4">
      {message ? (
      ...
      <section className="relationship-stage relationship-stage-v4">
        <div className="relationship-toolbar-v4">
          ...
          <button type="button" onClick={() => setHistoryOpen(true)} className="relationship-history-trigger" title="Histórico"><FiClock /></button>
        </div>

        <div className="relationship-mode-tabs relationship-mode-tabs-v4" role="tablist" aria-label="Tipos de vínculo">
        ...
        <section className="relationship-results relationship-results-v4">
