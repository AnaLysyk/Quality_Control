...
import { resolveRunRole, getRunMockRole, canEditRun } from "@/backend/rbac/runs";
...
// POST /api/v1/runs/[code]/[id]/complete
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const result = await client.completeRun(projectCode, runId);
    return apiOk(request, { data: result }, "OK", { extra: { data: result } });
  } catch (error) {
    const status = error instanceof QaseError ? error.status : 500;
    const msg = error instanceof Error ? error.message : "Erro ao completar run";
