...
export default function AccessDeniedState({
  ...
  return (
    <div className="mx-auto flex min-h-[55vh] w-full max-w-4xl items-center justify-center px-4 py-12">
      <section className="w-full rounded-3xl border border-[var(--tc-border,#d7deea)] bg-[var(--tc-surface,#ffffff)] p-8 text-center shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--tc-surface-alt,#f8fafc)] text-[var(--tc-primary,#011848)]">
          {loading ? <FiLoader className="h-6 w-6 animate-spin" /> : <FiLock className="h-6 w-6" />}
        </div>
        <h1 className="mt-5 text-2xl font-bold text-[var(--tc-text-primary,#0b1a3c)]">
