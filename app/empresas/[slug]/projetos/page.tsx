  ...
  return (
    <main className="min-h-screen bg-(--page-bg,#f8fafc) px-3 py-4 text-(--page-text,#0b1a3c) sm:px-5 lg:px-7">
      <div className="mx-auto flex w-full max-w-550 flex-col gap-5">
        ...
        <section className="rounded-[30px] border border-(--tc-border,#d7deea) bg-(--tc-surface,#ffffff) p-5 shadow-[0_18px_50px_rgba(15,23,42,0.06)] sm:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <h2 className="text-xl font-black tracking-[-0.03em] text-(--tc-text-primary,#0b1a3c)">Operações disponíveis</h2>
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <button type="button" onClick={() => void syncQase()} disabled={syncing} className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-(--tc-border,#d7deea) bg-white px-4 text-xs font-black uppercase tracking-[0.12em] text-(--tc-text-primary,#0b1a3c) transition hover:bg-(--tc-surface-alt,#f8fafc) disabled:opacity-60">
              ...
              {([
                ...
                <button
                  ...
                  className={`rounded-full border px-3 py-2 text-xs font-black uppercase tracking-[0.08em] transition ${
                    sourceFilter === value
                      ? "border-(--tc-primary,#011848) bg-(--tc-primary,#011848) text-white shadow-[0_10px_20px_rgba(1,24,72,0.16)]"
                      : "border-(--tc-border,#d7deea) bg-white text-(--tc-text-secondary,#4b5563) hover:bg-(--tc-surface-alt,#f8fafc)"
                  }`}
                >
                  {label}
                </button>
          ...
          {loading ? (
            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="h-64 animate-pulse rounded-3xl border border-(--tc-border,#d7deea) bg-(--tc-surface-alt,#f8fafc)" />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div className="mt-5 rounded-3xl border border-dashed border-(--tc-border,#d7deea) bg-(--tc-surface-alt,#f8fafc) px-4 py-12 text-center">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-(--tc-primary,#011848)">
                <FiFolder className="h-6 w-6" />
              </div>
              <h3 className="mt-4 text-lg font-black text-(--tc-text-primary,#0b1a3c)">Nenhum projeto encontrado</h3>
              ...
              <div className="mt-5 flex justify-center gap-2">
                <button type="button" onClick={() => setCreateOpen(true)} className="rounded-2xl bg-(--tc-primary,#011848) px-4 py-2 text-sm font-black text-white">Novo projeto</button>
                <button type="button" onClick={() => void syncQase()} className="rounded-2xl border border-(--tc-border,#d7deea) bg-white px-4 py-2 text-sm font-black text-(--tc-text-primary,#0b1a3c)">Sincronizar Qase</button>
              </div>
            </div>
          ) : (
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              {filtered.map((project) => {
                ...
                return (
                  <article key={makeProjectKey(project)} className={`overflow-hidden rounded-[28px] border bg-(--tc-surface-alt,#f8fafc) shadow-[0_14px_34px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_44px_rgba(15,23,42,0.09)] ${isActive ? "border-(--tc-primary,#011848) ring-4 ring-blue-950/5" : "border-(--tc-border,#d7deea)"}`}>
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-start gap-3">
                          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-(--tc-primary,#011848) shadow-sm">
                            {getProjectSource(project) === "qase" ? <FiLink className="h-5 w-5" /> : <FiFolder className="h-5 w-5" />}
                          </div>
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <h3 className="truncate text-xl font-black tracking-[-0.03em] text-(--tc-text-primary,#0b1a3c)">{project.name}</h3>
                      ...
                      <div className="mt-4 rounded-2xl border border-(--tc-border,#d7deea) bg-white p-3">
                        <p className="mb-2 text-[10px] font-black uppercase tracking-[0.16em] text-(--tc-text-muted,#64748b)">Módulos da operação</p>
                          ...
                          {OPERATION_MODULES.map((module) => {
                            ...
                            return (
                              <button
                                ...
                                className="group rounded-2xl border border-(--tc-border,#d7deea) bg-(--tc-surface-alt,#f8fafc) px-3 py-2 text-left transition hover:border-(--tc-primary,#011848) hover:bg-white"
                              >
                                <div className="flex items-center gap-2 text-xs font-black text-(--tc-text-primary,#0b1a3c)">
                                  <Icon className="h-3.5 w-3.5 text-(--tc-accent,#ef0001)" />
                                  {module.label}
                                </div>
                                <div className="mt-1 truncate text-[10px] font-semibold text-(--tc-text-muted,#64748b)">{module.description}</div>
                              </button>
                            );
                          })}
                        </div>
                    ...
                    <div className="flex flex-col gap-2 border-t border-(--tc-border,#d7deea) bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                      <div className="text-xs font-semibold leading-5 text-(--tc-text-secondary,#4b5563)">
                        Escopo: <strong>{companySlug}</strong> + <strong>{project.slug}</strong>
                      </div>
                      <button type="button" onClick={() => void openOperation(project, "dashboard")} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-(--tc-primary,#011848) px-4 py-2 text-xs font-black uppercase tracking-[0.1em] text-white transition hover:opacity-90">
      ...
      {createOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onClick={() => setCreateOpen(false)}>
          <div className="w-full max-w-2xl overflow-hidden rounded-[32px] border border-(--tc-border,#d7deea) bg-white shadow-[0_34px_100px_rgba(15,23,42,0.34)]" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="space-y-4 p-6">
              ...
              <label className="block text-sm font-semibold text-(--tc-text-primary,#0b1a3c)">
                ...
                />
                <span className="mt-1 block text-xs font-semibold text-(--tc-text-muted,#64748b)">
