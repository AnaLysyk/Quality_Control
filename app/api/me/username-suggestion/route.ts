...
export async function POST(req: Request) {
  try {
    ...
    if (!seed) {
      return NextResponse.json({ error: "Seed obrigatÃ³ria" }, { status: 400 });
    }
    ...
    const message = error instanceof Error && error.message.trim() ? error.message.trim() : "NÃ£o foi possÃ­vel gerar o login";
