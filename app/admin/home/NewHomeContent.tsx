...
export default function NewHomeContent() {
  ...
  function appendAssistantMessage(text: string) { updateCurrentSession((session) => ({ ...session, updatedAt: new Date().toISOString(), messages: [...session.messages, { id: `assistant-${Date.now()}`, role: "assistant", text }] })); }
    ...
    <section className="admin-brain-home relative min-h-[calc(100vh-7rem)] w-full overflow-y-auto overflow-x-hidden bg-transparent px-3 pb-44 pt-4 sm:px-6 sm:pb-36 lg:px-10">
      ...
      <div className="relative z-10 grid min-h-[420px] grid-cols-1 gap-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:gap-6 lg:items-start">
        ...
        <main className="admin-brain-copy mx-auto flex h-[calc(100vh-19rem)] min-h-[260px] w-full max-w-6xl flex-col justify-start gap-4 overflow-hidden pt-2 text-left sm:h-[calc(100vh-17rem)] lg:h-[calc(100vh-15rem)] lg:pt-12 lg:pr-12">
          <div className="admin-brain-chat-list space-y-3 overflow-y-auto pr-1 sm:pr-2">
            {messages.map((message, index) => { const isLatestAssistant = message.role === "assistant" && index === latestAssistantIndex; const displayText = isLatestAssistant ? typedAssistantText : message.text; return <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`admin-brain-message max-w-[92%] rounded-3xl px-4 py-3 sm:max-w-[82%] sm:px-5 sm:py-4 ${message.role === "user" ? "admin-brain-message-user text-right" : "admin-brain-message-assistant"}`}><p className="whitespace-pre-wrap text-sm font-semibold leading-relaxed sm:text-xl">{displayText}{isLatestAssistant ? <motion.span className="ml-1 inline-block h-5 w-[2px] translate-y-1 bg-red-500" animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 0.9 }} /> : null}</p></div></div>; })}
