...
export default function AdminBenchmarkPage() {
  ...
  const fallbackItems: BenchmarkItem[] = [
    {
      companySlug: "griaule",
      companyName: "Griaule",
      score: 92,
      ...
      <div className="mx-auto w-full max-w-7xl px-4 pt-4 sm:px-6 sm:pt-6 lg:px-10 lg:pt-10 space-y-6">
        ...
        <header className="space-y-2">
          ...
          <p className="text-sm sm:text-base text-(--tc-text-secondary,#4b5563)">
            Ranking de risco, qualidade e priorizacao de suporte.
          </p>
        </header>
        ...
        <section data-testid="benchmark">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {(data && Array.isArray(data.items) && data.items.length > 0 ? data.items : fallbackItems).map((item) => (
              <div
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-block w-2 h-2 rounded-full ${item.risk === "high" ? "bg-red-500" : item.risk === "medium" ? "bg-yellow-400" : "bg-green-500"}`}
                  ></span>
                  <span className="font-bold text-lg" data-testid="benchmark-company">
                    {item.companyName}
                  </span>
                  <span
                    className="ml-auto text-xs uppercase tracking-widest text-gray-500"
                    data-testid="benchmark-risk"
                  >
                    {item.risk}
                  </span>
                </div>
                <div className="flex flex-wrap gap-4 mt-2">
                  <div>
                    ...
                    <div className="text-xl font-bold" data-testid="benchmark-score">
                      {item.score}
                    </div>
                  </div>
                  <div>
          ...
          {!loading && data && Array.isArray(data.items) && data.items.length === 0 && (
            <div className="text-sm text-(--tc-text-muted,#6b7280)">Nenhum dado encontrado.</div>
          )}
        </section>
      </div>
    </div>
